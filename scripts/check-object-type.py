...
用法:
    ...
    `uv run scripts/check-object-type.py --check --quiet`

输出合约:
- `--check` 只控制退出码(有 `block` 命中时非 0); 不隐含静默.
...
def _render_text_report(*, repo_root: Path, hits: Sequence[_Hit], show_allow_details: bool = False) -> str:
    ...
    if hits:
        ...
        if allowed and not show_allow_details:
            lines.append("")
            lines.append("allow 明细默认省略; 如需展开,运行 `uv run scripts/check-object-type.py --show-allow-details`.")
    else:
        lines.append("")
