...
def _dirs_in_archive(tag: str, *, root: Path) -> Set[str]:
    for archive_prefix in _ARCHIVE_PREFIXES:
        out = _try_run_git(["ls-tree", "-d", "--name-only", tag, archive_prefix], cwd=root)
        if out is None:
            continue
...
def _render_example_snippet(change_id: str, priority: int) -> Optional[List[str]]:
    ...
    if "books-resources" in cid or "io-books-resources" in cid:
        return [
            ...
            "resources:",
            "  books:",
            "    report: {xlsx_file: {path: {$init_var: out_xlsx}}}",
            "outputs_defaults:",
            "  to: {book: report}",
            "outputs:",
            "  - id: orders",
...
def _read_change_file_at_tag(tag: str, change_id: str, filename: str, *, root: Path) -> Optional[str]:
    for archive_prefix in _ARCHIVE_PREFIXES:
        relpath = "{}{}/{}".format(archive_prefix, change_id, filename)
        text = _read_file_at_tag(tag, relpath, root=root)
        if text is not None:
            return text
    return None
...
def _build_change(tag: str, change_id: str, *, root: Path) -> Optional[_Change]:
    proposal_text = _read_change_file_at_tag(tag, change_id, "proposal.md", root=root)
    if proposal_text is None:
        # 兼容极端情况：`archive` 中没有 `proposal.md` 时再尝试 `design.md`（仍只读快照，不读 `patch`）。
        proposal_text = _read_change_file_at_tag(tag, change_id, "design.md", root=root)
        if proposal_text is None:
            return None
...
def _render_notes(
    ...
    *,
    ...
    if not new_changes:
        lines.append("- 本版本没有新增 archived llmanspec 提案，主要是实现/修整。")
    else:
...
def _parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="按 archived llmanspec 提案+commit messages 批量 create/edit GitHub Releases。")
