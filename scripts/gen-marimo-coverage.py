...
def _path_value(path: Optional[Path], *, root: Path) -> Optional[str]:
    if not path:
        return None
...
def _report_data(root: Path, rows: Sequence[_Row], warnings: Sequence[str]) -> Dict[str, Any]:
    total = len(rows)
        ...
        report_rows.append(
            {
                "kind": r.kind,
                "item": r.item_id,
                "notebook": _path_value(r.notebook, root=root),
                "ssot": _path_value(r.ssot, root=root),
                "gate": r.gate,
                "pytest": _path_value(r.pytest, root=root),
                "ok": bool(r.ok),
    ...
    return {
        "generated_by": "scripts/gen-marimo-coverage.py",
        "summary": {"ok": ok_count, "total": total},
        "warnings": list(warnings),
        "rows": report_rows,
        "regenerate": [
            "just gen-marimo-coverage",
            "uv --preview-features extra-build-dependencies run python scripts/gen-marimo-coverage.py",
        ],
    }


def _encode_toon(*, root: Path, data: Any) -> str:
    tool = root / "scripts" / "tool-toon.py"
    if not tool.exists():
        raise FileNotFoundError("缺少工具: `{}`".format(tool))
    ...
    if proc.returncode != 0:
        raise RuntimeError("`tool-toon` 执行失败: `{}`\n{}".format(" ".join(cmd), (proc.stderr or proc.stdout or "").strip()))
    return proc.stdout
...
def _parse_args(argv: Optional[Sequence[str]]) -> argparse.Namespace:
    ...
    p.add_argument(
        "--output",
        help="输出路径(默认: notebooks/marimo/marimo_coverage.gen.toon)。",
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    content = _encode_toon(root=root, data=_report_data(root, rows, warnings))

    output = Path(args.output) if args.output else root / "notebooks" / "marimo" / "marimo_coverage.gen.toon"
