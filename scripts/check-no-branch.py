...
# ruff: noqa: T201
# force-en
"""扫描源码中的 `# pragma: no branch` 使用.

设计目标:
- `--cov-branch` 启用后,`# pragma: no branch` 是“治理例外”,必须可审阅、可追踪.
- 防止通过随意添加 `no branch` 绕过 branch coverage gate.
- 允许对确属必要的例外做显式标记:
  - 行级: `# pragma: allow-no-branch <reason>`
  - 文件级: `# pragma: allow-no-branch-file <reason>`
- allow pragma 属于治理标记;在测试或结构重构完成前不要直接删除.

用法:
    `uv run scripts/check-no-branch.py`
    `uv run scripts/check-no-branch.py --json`
    `uv run scripts/check-no-branch.py --check`
    `uv run scripts/check-no-branch.py --check --quiet`
...
_NO_BRANCH_MARK = "pragma: no branch"
_ALLOW_MARK = "pragma: allow-no-branch"
_ALLOW_FILE_MARK = "pragma: allow-no-branch-file"
_DEFAULT_REL_ROOTS = (Path("src") / "scalim", Path("tests"), Path("scripts"))
_DEFAULT_TEXT_REPORT_REL = Path(".tmp") / "artifacts" / "no-branch.report.txt"
_DEFAULT_JSON_REPORT_REL = Path(".tmp") / "artifacts" / "no-branch.report.json"
...
def _scan_file(path: Path) -> list[_Hit]:
    ...
    for token in tokenize.generate_tokens(io.StringIO(source).readline):
        if token.type != tokenize.COMMENT:
            continue
        stripped = token.string.lstrip("#").strip()
        if _NO_BRANCH_MARK not in stripped:
            continue
...
def _render_text_report(*, repo_root: Path, hits: Sequence[_Hit], show_allow_details: bool = False) -> str:
    ...
    lines.extend(
        [
            "",
            "规避建议:",
            "  1. 可测试分支优先补测试,不要先加 `# pragma: no branch`.",
            "  2. 确属必要的例外,在同一行或上一行加 `# pragma: allow-no-branch <reason>`.",
            "  3. 文件整体承担框架/兼容职责时,在文件头注释区加 `# pragma: allow-no-branch-file <reason>`.",
    ...
    if hits:
        ...
        if allowed and not show_allow_details:
            lines.extend(["", "allow 明细默认省略; 如需展开,运行 `uv run scripts/check-no-branch.py --show-allow-details`."])
    else:
        lines.extend(["", "未发现 `# pragma: no branch`."])
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="扫描源码中的 `# pragma: no branch` 使用.")
    ...
    parser.add_argument("--no-artifacts", action="store_true", help="不自动写入 `.tmp/artifacts/no-branch.report.{txt,json}`.")
