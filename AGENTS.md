...
- **YAML authoring vs Python policy** (workflow/books 迭代方向):
  ...
  - Agent-facing migration: `agentdev/skills/scalim-yaml-dsl/references/upgrades/2026-07-12-book-write-policy-python-ssot.md`；统一 book identity 见 `.../2026-07-13-unified-xlsx-book-kind.md`（推荐 `resources.books.<id>.xlsx` 可选 `path`；旧 `xlsx_file`/`xlsx_memory` 为 deprecated 别名）；IR pathful/pathless 见 `.../2026-07-13-normalize-xlsx-book-ir-path-presence.md`。
  - `allow_formulas` / `encoding` may remain YAML for now (usually static).
  - Archived track: `llmanspec/changes/archive/2026-07-12-c20-book-write-policy-python-ssot/`、`.../2026-07-12-c30-workflow-shared-book-memory/`、`.../2026-07-13-c15-decide-xlsx-memory-book-role/`、`.../2026-07-13-c20-add-unified-xlsx-book-kind/`、`.../2026-07-13-c25-normalize-xlsx-book-ir-path-presence/`；硬删旧 kind 别名见 draft `llmanspec/changes/c999-remove-deprecated-xlsx-file-memory-kinds/`；后续 shared-book 优化见 `llmanspec/futures/xlsx-file-numeric-type-loss/future.md`。
