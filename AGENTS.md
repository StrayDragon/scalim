...
- **YAML authoring vs Python policy** (workflow/books 迭代方向):
 ...
 - **Output write layout（c30）**：`OutputWriteLayout`（`row_stream`/`column_hold`/`column_window`）仅 Python（`DemandRunRuntimeOptions` / `ExecutionRequest`）；禁止 YAML；未设时由 `streaming`+`excel_column_residency` 推导且默认路径行为不变。
