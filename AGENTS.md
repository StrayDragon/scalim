...
- **YAML authoring vs Python policy** (workflow/books 迭代方向):
  ...
  - Book **write strategy** is **Python SSOT** via `ResourcesPolicy` / `BookWritePolicy` on `WorkflowRunOptions` / `DemandRunOptions` (builtin defaults when omitted).
 ...
 - **Output write layout（c30）**：`OutputWriteLayout`（`row_stream`/`column_buffered`/`column_chunked`）仅 Python（`DemandRunRuntimeOptions` / `ExecutionRequest`）；禁止 YAML；未设时由 `streaming`+`excel_column_residency` 推导且默认路径行为不变。
...
- Examples/notebooks SSOT（单一套件）: `notebooks/marimo/demo_big_data_report/` 三条轨道 —— `chapters_of_yaml_dsl/`(声明面, 含 README 最小示例 `ch005_yaml_dsl_min.py` + `declared_yaml_dsl/min_report.yaml`)、`chapters_of_ir/`(装配面 + public API 覆盖, 含 README 最小示例 `ch010_basics.py`/`ch020_memory_compare.py`)、`chapters_of_scenarios/`(场景面 `ch210`–`ch260`)；`scalim_misc` 只放 loader 模块 + 对拍零件 + runner 基础设施，其余 in-cell；注入/图表生成器在 `packages/scalim-misc/src/scalim_misc/readme_examples_gen.py` + `readme_charts_gen.py`（`just examples` + `just gen-readme-examples`；合约 `governance-readme-examples` + `examples-marimo` r989/r198/r180）
