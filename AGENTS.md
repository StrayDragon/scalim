...
- **YAML authoring vs Python policy** (workflow/books 迭代方向):
  ...
  - 边界盘点 / 灰区（`lookup_chunk_size`、两套 `cache_mode`）SSOT：`llmanspec/changes/c40-yaml-runtime-policy-boundary/inventory.md`；agent：`agentdev/skills/scalim-yaml-dsl/references/yaml-runtime-policy-boundary.md`。
