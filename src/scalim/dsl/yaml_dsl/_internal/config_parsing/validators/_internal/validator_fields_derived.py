...
class ValidatorFieldDerivedMixin(ValidatorFieldBaseMixin):
    def _collect_derived_fields(
        ...
        if raw_fields is None:
            self._add_error(errors, f"'{_F.FIELDS}' must be a dictionary", path=_F.FIELDS)
            return
        ...
        for field_id_raw, field_data_raw in raw_fields.items():
            ...
            if field_dict is None:
                self._add_error(errors, f"Field '{field_id}' must be a dictionary", path=f"fields.{field_id}")
                continue
            ...
            if not has_compute and not has_call_by:
                self._add_error(
                    errors,
                    f"Derived field '{field_id}' must declare compute/call_by",
                    path=f"fields.{field_id}",
                )
                continue
            self._validate_field_id_not_reserved(field_id, errors, path=f"fields.{field_id}")
    ...
    def _validate_no_derived_source_overlap(
        ...
        for field_id, defs in defs_by_id.items():
            ...
            if FIELD_KIND_DERIVED in kinds and FIELD_KIND_SOURCE in kinds:
                self._add_error(
                    errors,
                    f"Field '{field_id}' is defined as both source and derived; use unique field_id",
                    path=f"fields.{field_id}",
                )
    ...
    def _validate_derived_field(self, field_id: str, field_data: dict[str, Any], errors: list[ValidationIssue]) -> None:
        ...
        if has_compute == has_call_by:
            if has_compute:
                msg = f"Derived field '{field_id}' must not declare both '{_F.COMPUTE}' and '{_F.CALL_BY}'"
            else:
                msg = f"Derived field '{field_id}' must declare '{_F.COMPUTE}' or '{_F.CALL_BY}'"
            self._add_error(errors, msg, path=f"fields.{field_id}")
            return

        if _F.DEPENDS_ON in field_data:
            self._add_error(
                errors,
                f"Derived field '{field_id}' does not allow '{_F.DEPENDS_ON}'; dependencies are inferred from '{_F.COMPUTE}'/'{_F.CALL_BY}'",  # noqa: E501
                path=f"fields.{field_id}.{_F.DEPENDS_ON}",
            )
    ...
    def _validate_derived_field_compute(
        ...
        if not isinstance(compute_val, str):
            self._add_error(
                errors,
                f"Derived field '{field_id}' compute must be a string",
                path=f"fields.{field_id}.{_F.COMPUTE}",
            )
            return
    ...
    def _validate_derived_field_call_by(
        ...
        if not isinstance(call_by_val, str):
            self._add_error(
                errors,
                f"Derived field '{field_id}' call_by must be a string",
                path=f"fields.{field_id}.{_F.CALL_BY}",
            )
            return
        if not call_by_val:
            self._add_error(
                errors,
                f"Derived field '{field_id}' call_by must not be empty",
                path=f"fields.{field_id}.{_F.CALL_BY}",
            )
            return
        ...
        try:
            ...
            self._add_error(
                errors,
                f"Derived field '{field_id}' has invalid call_by: {exc}",
                path=f"fields.{field_id}.{_F.CALL_BY}",
            )
            return
        ...
        if not used_fields:
            self._add_error(
                errors,
                f"Derived field '{field_id}' call_by has no field dependencies",
                path=f"fields.{field_id}.{_F.CALL_BY}",
            )
    ...
    def _validate_derived_expression(
        ...
        if not compute_expr:
            self._add_error(
                errors,
                f"Derived field '{field_id}' compute must not be empty",
                path=f"fields.{field_id}.{_F.COMPUTE}",
            )
            return
        ...
        try:
            ...
            self._add_error(
                errors,
                f"Derived field '{field_id}' has invalid compute expression: {exc}",
                path=f"fields.{field_id}.{_F.COMPUTE}",
            )
            return
        ...
        if not dependencies and not is_constant_compute_expression(compute_expr):
            self._add_error(
                errors,
                f"Derived field '{field_id}' compute has no field dependencies; only pure literal expressions are allowed",
                path=f"fields.{field_id}.{_F.COMPUTE}",
            )
        ...
        for field_id, depends_on, dep_path in derived_fields_with_deps:
            for dep in depends_on:
                ...
                if not matched:
                    self._add_error(
                        errors,
                        f"Derived field '{field_id}' depends on unknown field '{dep_str}'",
                        path=dep_path,
                    )
                elif len(matched) > 1:
                    self._add_error(
                        errors,
                        f"Derived field '{field_id}' depends on ambiguous field '{dep_str}'",
                        path=dep_path,
                    )
    ...
    def _resolve_derived_dependencies(self, *, field_id: str, field_dict: dict[str, Any]) -> tuple[list[str], str]:
        ...
        if compute_expr:
            return extract_compute_dependencies(compute_expr), f"fields.{field_id}.{_F.COMPUTE}"

        call_by_raw = field_dict.get(_F.CALL_BY)
        call_by_expr = str(call_by_raw or "")
        if call_by_expr:
            return extract_call_by_dependencies(call_by_expr), f"fields.{field_id}.{_F.CALL_BY}"
