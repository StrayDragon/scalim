...
VariableValue = str | int | float | bool | list[Any] | dict[str, Any] | None
...
ExpressionResult = str | int | float | bool | list[Any] | dict[str, Any] | None
""" 表达式求值结果类型 """


FilterArg = str | VariableValue
""" 过滤器参数类型 """
...
class ControlNode(TypedDict):
    ...
    content: str
    subtype: Literal["if", "for", "set", "macro", "end"]
    condition: str | None  # `if` 条件的表达式
    target: str | None  # `for` 循环的目标变量
    iterable: str | None  # `for` 循环的可迭代对象
