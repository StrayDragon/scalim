"""Row-wise compute fusion (c20): 同 deps 组按行一次读依赖、依次算字段.
...
def active_fusion_members(group: ComputeFusionGroup, late_fields: Any) -> Tuple[str, ...]:
    """去掉当前 runtime late 后的组员(写出路径跳过 late)."""
    if not late_fields:
        return group.field_keys
    late_set = set(late_fields) if not isinstance(late_fields, (set, frozenset)) else late_fields
    return tuple(fk for fk in group.field_keys if fk not in late_set)


def _execute_fused_dense(  # noqa: C901  # pragma: allow-c901 plan: c20-fused-dense
    *,
    group: ComputeFusionGroup,
    ...
    try:
        ...
        for field_key, _, _ in field_plans:
            storage = context.dense_prepare_write_storage(field_key)
            if storage is None:
                return False
        ...
        for row_id in batch_row_nth:
            ...
            if deps_len == 0:
                ...
                st1 = dep_storages[1]
                d0 = None if st0 is None or st0.present[idx] == 0 else st0.values[idx]
                d1 = None if st1 is None or st1.present[idx] == 0 else st1.values[idx]
                dep_args = (d0, d1)
            else:
                args_list = []
            ...
            dep_payload = None  # type: Optional[Dict[str, Any]]
...
def execute_fused_compute_group(
    *,
    group: ComputeFusionGroup,
    ...
    """对 `field_keys`(size>=2) 执行 row-wise 融合;依赖读取一次/行."""
    deps = group.deps
    compute_mode = runtime.guardrails.effective_compute_mode()
    guardrails_enabled = runtime.guardrails.enabled
