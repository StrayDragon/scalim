        ...
        *,
        ...
        if workers is not None:
            if isinstance(workers, bool) or not isinstance(workers, int):
                msg = "LookupChunking.sized(max_chunk_workers=...) must be an int or None"
                raise TypeError(msg)
            if int(workers) < 1:
                msg = "LookupChunking.sized(max_chunk_workers=...) must be >= 1 when provided"
                raise ValueError(msg)
            workers = int(workers)
