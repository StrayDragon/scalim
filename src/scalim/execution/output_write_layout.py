...
闭集 `OutputWriteLayout` 统一解释行流式 / 列缓冲 / 列分块刷盘.
...
class OutputWriteLayout(StrEnum):
    ...
    COLUMN_BUFFERED = "column_buffered"
    COLUMN_CHUNKED = "column_chunked"
...
def resolve_output_write_layout(
    *,
    ...
    推导表见 `runtime-output-write-layout`.
    """
    ...
    if streaming:
        # 历史:`excel` + `CHUNKED` + `streaming` 在工厂 `fail-fast`;此处仍推导 `row_stream`,
        # 由 `validate_output_write_layout_combos` / 工厂保留同等拒绝.
        return OutputWriteLayout.ROW_STREAM

    if fmt == "excel" and excel_column_residency is ExcelColumnResidency.CHUNKED:
        return OutputWriteLayout.COLUMN_CHUNKED

    # `csv` 忽略 `residency`(含 `CHUNKED`)→ `column_buffered`,与今日 `_create_file_sink` 一致.
    return OutputWriteLayout.COLUMN_BUFFERED
...
def validate_output_write_layout_combos(
    *,
    ...
    if has_output_composition and excel_column_residency is ExcelColumnResidency.CHUNKED:
        msg = (
            "`ExcelColumnResidency.CHUNKED` 与 `output_composition`(`YAML` books/多输出行组合)互斥."
            " 组合层仅为行流式写出;`CHUNKED` 只适用于列式 `IR` 文件 `sink`"
            "(`format=excel` 且 `streaming=False`)."
            " 请改用 `BUFFERED`,或改用手写 `StreamingColumnExcelSink` / 非 `composition` 列式路径."
        )
        raise ValueError(msg)
    ...
    if not layout_explicit and fmt == "excel" and streaming and excel_column_residency is ExcelColumnResidency.CHUNKED:
        msg = (
            "`ExcelColumnResidency.CHUNKED` 仅适用于列式 `Excel` 文件 `sink`"
            "(`OutputSpec.format=excel` 且 `streaming=False`)."
            " 当前为行式写出(`streaming=True`);请改用 `BUFFERED`,或关闭 `streaming` 后再设 `CHUNKED`."
        )
        raise ValueError(msg)
