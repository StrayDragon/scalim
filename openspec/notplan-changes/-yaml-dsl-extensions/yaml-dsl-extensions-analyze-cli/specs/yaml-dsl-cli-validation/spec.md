...
约束:
- JSON 输出 MUST 包含 `extensions_errors: Issue[]` 与 `extensions_warnings: Issue[]`
- JSON 输出 MAY 包含 `extensions_summary: object`(用于对拍;结构由 `ExtensionHost.summary` 决定)

#### Scenario: JSON 输出包含 extensions_errors/extensions_warnings
- **GIVEN** `extensions.analyze` 产生至少一个 warning
- **WHEN** 用户运行 `PROJECT_CLI_NAME yaml-dsl validate <yaml> --resolve-extensions --json`
- **THEN** JSON 输出 MUST 包含 `extensions_warnings` 与 `extensions_errors` 字段
- **AND** `extensions_warnings` MUST 非空
