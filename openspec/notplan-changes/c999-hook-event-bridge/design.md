...
### P1: `EventDescriptor` 是展示元数据,非序列化契约

`catalog.py` 的 `EventDescriptor` 只含 `name/summary/key_fields/volume/payload_policy/payload_type`(类名字符串)。bridge 仍需单独建立 payload_type → 字段映射 + 编码策略,descriptor 不携带 schema 或版本信息。

### P2: `Event.to_dict` 遇到非 dataclass payload 会退化

`event.py` 的 `to_dict` 对 payload 仅在 `is_dataclass(payload)` 时调用 `asdict`,其他情况原样传递。`FieldComputeEvent.result` 为 `Any`,`ErrorEvent.error` 为 `Exception` 对象——跨进程桥接时 JSON 序列化会失败。需要引入 payload 规范化层。

### P3: `ErrorEvent` 携带活的 `Exception` 对象

`asdict(ErrorEvent)` 会尝试序列化异常对象,这在 JSON/MQ 等传输层不可移植。bridge 需要将 Exception 转换为 `{type, message, traceback_str}` 等可序列化形式。

### P4: `InstrumentationHub.emit` 完全同步

从 `hub.emit()` → `hook_manager.emit_typed()` → `observer_manager.emit_event()` → `hook_manager.emit_on_event()` 全部在调用者线程同步执行。bridge 必须自带异步分发（线程/队列）,否则 I/O 操作会阻塞 pipeline 热路径。

### P5: Hook 注册涉及锁和分发策略

`HookManager` 内部通过 `_get_typed_handler_pairs` + `HookDispatchStrategy` 管理回调。bridge 如果作为 Hook 注册,需要考虑:多线程 emit 下的安全性、动态注册/注销的时序、adaptive 并发后端下的事件顺序不确定性。
