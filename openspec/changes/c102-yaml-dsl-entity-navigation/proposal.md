...
- 不处理 Python 引用 / builtin callable / imports alias（分别由现有能力和 `yaml-dsl-lsp-sugar-support` 负责）。
- 不实现 `textDocument/documentSymbol`（YAML language server 已提供通用 YAML symbols，本提案不重复实现）。
...
依赖 `yaml-dsl-lsp-resolution-infra` 的多 Location 返回框架和 Resolution Trace。
...
- server：`packages/scalim-yaml-dsl-lsp/src/scalim_yaml_dsl_lsp/server.py`
  - 注册 `textDocument/definition`、`textDocument/completion`、`textDocument/hover` handler
- specs（后续转正时）：
