...
- 建议在 `outputs-parser-split` 之前落地：先把 producer keys 的 SSOT 统一，再做 outputs parser 的 staged 拆分，可以显著降低重构时的改动半径与漂移风险。
- 建议与 `ordered-unique-ssot` 同期或之前落地（两者均属于 outputs 体系的 SSOT 基建）。
