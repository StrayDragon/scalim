## 0. Dependencies（先稳定语义，再做结构拆分）

- [ ] 0.1 确保 `yaml-path-escape-hardening` 已落地并稳定（workflow demand path / alias 解析与 allow-roots 口径不再变化）
- [ ] 0.2 确保 `yaml-template-vars-sandbox` 已落地并稳定（workflow YAML 模板预编译的安全边界不再变化）
