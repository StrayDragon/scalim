...
#### Scenario: scalim.yaml lsp.python_roots influences discovery output
- **GIVEN** `scalim.yaml` 配置了 `yaml_dsl.lsp.python_roots`
- **WHEN** 编辑器对该项目内的 YAML 执行 discovery
- **THEN** discovery 输出的 `python_roots` MUST 反映该配置
