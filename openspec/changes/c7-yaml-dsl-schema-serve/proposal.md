...
当前 YAML DSL 的 `# yaml-language-server: $schema=...` 头部通常需要引用仓库内的相对路径(例如 `../../../../src/scalim/dsl/by_yaml/schema/demand.gen.json`)或本机 `.venv/site-packages/...` 的绝对路径。
...
- **New**: `scalim-cli yaml-dsl upsert-lsp-comment` — 批量检查并按需写入/更新 YAML 文件的 `yaml-language-server` `$schema` 头部
  - 参数:
  ...
  - 行为:
    - 若文件头部存在 `# yaml-language-server: $schema=...`,则更新为期望值(不匹配才写入)
    - 若不存在,则插入到文件第一行
    - 输出哪些文件被修改/哪些文件已是最新
...
- 用户工作流:
  - 本地运行 `scalim-cli yaml-dsl schema-serve` 后,用户可在 YAML 顶部使用 `# yaml-language-server: $schema=http://127.0.0.1:62831/<schema>.gen.json`
