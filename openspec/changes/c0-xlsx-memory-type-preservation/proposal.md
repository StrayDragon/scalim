...
- 将 workflow-managed temp outputs 中,供 `xlsx_memory` 写节点消费的中间态从 `CSV` 等价字符串化语义升级为 typed artifact 或等价 typed metadata 驱动路径,避免热点链路上的“字符串化 -> 恢复”往返开销。
...
- `workflow-managed-temp-outputs`: 对供 `xlsx_memory` 写节点消费的 workflow-managed 中间态,要求避免 `CSV` 等价字符串化热点路径,改为 typed artifact 或等价 typed transport。
