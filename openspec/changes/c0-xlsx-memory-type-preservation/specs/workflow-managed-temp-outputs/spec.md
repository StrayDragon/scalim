...
- 当下游 consumer 为 `xlsx_memory` 写节点时,系统 MUST 为对应 output 提供按 output 粒度的 typed in-memory artifact 或等价 typed transport
...
- **THEN** 系统 MUST 为该 output 提供可供写入节点消费的 typed in-memory artifact 或等价 typed transport
