# yaml-dsl-output-overrides Delta Specification (typed overrides)
...
本 change 仅承诺明细输出(detail)的最小子集,至少包含:
- `name`
- `fields`(有序 field_id list)
- `to`(例如 `{book,sheet}` 绑定) 或 `container`(csv 文件输出) 二选一
- `write`(可选;仅在 `to` 绑定输出时允许)
...
- **GIVEN** demand YAML 未声明 `outputs`
- **WHEN** 调用方在 `run/compile` 中提供 `RunOverrides.outputs` 且该 output 包含 `container.path` 与 `fields`
...
- **WHEN** 调用方提供的 typed `RunOverrides.outputs` 结构非法(例如缺少 `name/fields`,或 `to/container` 互斥关系非法)
- **THEN** 编译 MUST fail-fast
...
系统 MUST 在 `RunOverrides` 中提供 IO-only 覆盖能力,用于在不修改 demand/workflow YAML 的前提下,覆盖 books 资源与默认输出绑定(仅 IO 层,不触及输出定义层)。

最小集合:
- `RunOverrides.resources`(typed dataclasses; 至少支持 `resources.books`)
...
- overlay MUST 仅允许覆盖 IO 层字段(例如 `books.*.path/budget/export_xlsx/write_defaults/allow_formulas/write_lock`)
...
**Reason**: YAML-shaped `dict/list[dict]` 作为公共契约过于脆弱,导致下游把结构细节固化为代码并在 schema/实现演进时高频破坏;同时框架内部需要维护多处 dict 解析与校验逻辑,增加漂移风险与维护成本。

**Migration**: 使用 typed dataclasses:
- `RunOverrides.outputs=(OutputOverride(...),)`
- 或使用 `RunOverrides` 的标准 `@classmethod` 工厂方法

#### Scenario: legacy YAML-shaped overrides is no longer supported
- **WHEN** 调用方传入 `RunOverrides(outputs=[{...}])`
- **THEN** 编译 MUST fail-fast
- **AND** 错误信息 MUST 提示迁移到 typed dataclasses
