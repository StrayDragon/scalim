...
- 任何**全文件生成物** MUST 在文件名中包含 `.gen.`(例如 `*.gen.md`/`*.gen.json`/`*.gen.yaml`/`*.gen.html`)
...
#### Scenario: 标记区块可被精确替换
- **WHEN** 生成器刷新一个带 `BEGIN/END SCALIM-GEN` 的文档
...
- **WHEN** 开发者运行文档生成入口(例如 `just gen` 或 `just gen-docs`)
- **THEN** 所有受控 `*.gen.*` 与 `SCALIM-GEN` 注入区块 MUST 被一次性刷新到最新
...
- **THEN** 文档 MUST 包含对 `.gen.*` 文件与 `SCALIM-GEN` 注入区块的规则说明与生成入口提示
