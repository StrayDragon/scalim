...
- **生成边界不统一**: 既有 `.gen.json/.gen.md` 的全文件生成,也有 `<!-- BEGIN SCALIM-GEN:... -->` 的区块注入,但缺少“通用且强约束”的仓库级规范,导致 agent/贡献者很难稳定判断“该改哪里/该跑什么生成器”。
...
- 统一 **生成物命名与鉴别**:
  ...
  - 对手工文档中的“受控注入区块”,统一使用 `<!-- BEGIN SCALIM-GEN:<id> -->` / `<!-- END SCALIM-GEN:<id> -->`
...
- 增加自动化兜底与 CI/QA 漂移防线:
  - 把文档生成/注入统一收敛到 `just gen` 或独立 `just gen-docs`(由设计阶段确定)
...
- （可选）为 skill 与文档提示引入 prompt 评测流水线(如 promptfoo),以 regression/质量门禁方式持续调优。

## Capabilities

### New Capabilities
- `doc-governance`: 定义仓库文档分层、生成边界(`*.gen.*` + `SCALIM-GEN` 区块)、更新工作流与漂移校验的规范。
