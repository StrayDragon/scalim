...
- 列出“可能导致错误”的重叠/复用条件（用于后续判断是否需要更强护栏或签名化 key）。
 - 将“loader SHOULD be idempotent（或等价约束）”从建议提升为 `source-cache` 相关规范的显式条款（至少 SHOULD），以匹配并发/重复加载的现实边界。
 - 明确后置治理方向：为 `PreloadCache` 增加可选 guardrail（例如当检测到跨不同 signature 复用同一 `source_id` 时 fail-fast/告警），但作为后置 change 落地（避免本 change 变成“大而杂”的实现变更）。

## Sequencing / Dependencies

- 建议与 `preload-cache-inflight-wait-diagnostics` 搭配推进：本变更负责边界澄清与复现口径，后者负责“卡住时可定位”的诊断能力，两者共同降低并发场景的排障成本。
