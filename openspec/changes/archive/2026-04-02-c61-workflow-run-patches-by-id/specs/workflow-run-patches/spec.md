...
为 `scalim.dsl.by_yaml.run_workflow(...)` 提供一个可扩展的 per-run runtime patch 入口,以支持在同一张 workflow DAG 中对不同 run id 应用不同的运行期策略(例如 `batch_size`、`components`、`RunOverrides`、`guardrails` 等),同时保持 runtime-policy boundary: 这些 knobs 不回流到 YAML 主线,且安全边界(allowlist 等)不允许被 per-run patch 覆盖。
