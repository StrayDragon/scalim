...
- [x] 1.1 提炼统一的 callable preflight SSOT API（callsite `location/path` + 签名绑定校验 + 诊断格式）
...
- [x] 1.2（已完成）在派生字段 `fields.*.call_by` 编译期执行签名绑定校验并 fail-fast
- [x] 1.3（已完成）在聚合后派生字段 `outputs[*].aggregate.fields.*.call_by` 编译期执行签名绑定校验并 fail-fast
- [x] 1.4 为 `sources.*.normalize.call_by` 增加编译期 signature precheck（至少接受 `result` 位置参数,可选 `ctx`）
...
- [x] 1.5 为 loader retry `should_retry(exc, ctx)` 增加编译期 signature precheck（当 `enabled=true` 且回调可 introspect 时）
...
- [x] 1.6 为 `compute` 表达式中的 `SAFE_FUNCTIONS` 调用增加“可推理的调用形态/参数个数”预检查（基于 AST + `inspect.signature.bind`）
...
- [x] 1.7 为 loader `params` kwargs 模板增加编译期 signature precheck（`main_source.params` / `sources.*.params`; 当签名可 introspect 时用 `bind(**kwargs_keys)` 校验未知 key / 缺失必填）
  - Touchpoints:
...
- [x] 2.1 明确 preflight 错误的异常类型与上抛边界（配置/编译错误；不得进入运行期 guardrails quiet/吞错语义）
  - DoD:
...
- [x] 2.2 统一错误文案格式（包含 `location`、引用、签名、绑定失败原因、以及可照抄的改写提示）
  - DoD:
...
- [x] 3.1（已完成）单元测试覆盖 `call_by` 签名绑定校验的关键分支（keyword-only / kwargs / 无签名可取等）
- [x] 3.2 增加 `normalize.call_by` 的编译期预检查测试（keyword-only result / 接受 ctx 的不同签名形态）
  - DoD: 覆盖 `(result)` / `(result, ctx)` / `(result, *, ctx)` / `*args/**kwargs` / 不可 introspect 的跳过策略
- [x] 3.3 增加 `compute` 内置函数调用形态预检查测试（例如 `len(x, y)` / `dec(a, b)` 必须编译期失败）
  - DoD: 同时覆盖“通过用例”（例如 `Decimal('0.1')` / `dec(x)`）与“拒绝用例”（arity mismatch）
- [x] 3.4（已完成）补充 notebooks 场景样例并纳入集成测试门禁（坏例子必须 fail-fast；好例子可运行且产出行数稳定）
  - 范围: 仅覆盖 `call_by` keyword-only 的 fast-fail（`yaml_dsl_call_by_keyword_only`）
- [x] 3.5 增加 `should_retry` 的编译期预检查测试（keyword-only / 缺少 ctx / 缺少 exc 等）
  - DoD: `enabled=true` 时触发预检查；`enabled=false` 不强制要求 `should_retry`（保持现有语义）
- [x] 3.6 增加 loader `params` 的编译期预检查测试（未知 kw / 缺失必填 kw）
  - DoD: 覆盖 `main_source.params` 与 `sources.*.params` 两类入口
  - DoD: 覆盖 `**kwargs` loader 允许未知 kw 的路径
- [x] 3.7 notebooks: 新增 `yaml_dsl_compute_builtin_arity_mismatch`（bad YAML 必须 compile 期失败；good gate 复用 `yaml_dsl_support`/`yaml_dsl_ecommerce`）
- [x] 3.8 notebooks: 新增 `yaml_dsl_normalize_call_by_signature_mismatch`
...
- [x] 3.9 notebooks: 新增 `yaml_dsl_loader_params_signature_mismatch`（bad: params unknown/missing；good gate 复用 `yaml_dsl_support`）
- [x] 3.10 notebooks: 新增 `yaml_dsl_should_retry_signature_mismatch`（注入 bad should_retry,要求 compile 期失败；good gate 复用 `yaml_dsl_ads`）
- [x] 3.11 集成门禁: 更新 `notebooks/.../chapters_of_yaml_dsl/registry.py` 与 `tests/integration/test_demo_big_data_report_chapters.py` 选中上述新 chapter ids
...
- [x] 4.1 若新增/调整 marimo chapters: 重新生成 `notebooks/marimo/marimo_coverage.gen.toon`
