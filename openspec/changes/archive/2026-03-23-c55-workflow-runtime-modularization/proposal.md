...
- 建议在路径/模板相关语义稳定后再做大拆分：
  - `yaml-path-escape-hardening`（workflow paths/alias 语义将被强化，先落地更易定位回归）
  - `yaml-template-vars-sandbox`（workflow YAML 也会走模板预编译，先收敛安全边界避免重构中重复改动）
- 本变更必须坚持“护栏先行”：先补齐稳定入口 smoke 与关键不变量测试，再迁移文件/拆分模块。
