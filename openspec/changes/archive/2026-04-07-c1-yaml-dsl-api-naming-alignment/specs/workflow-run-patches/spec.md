...
**Reason**：per-run patch 参数已收敛为 `run_options_patches_by_run_id`；旧的 `run_patches_by_id` 命名不再作为公共契约保留。

### Requirement: unknown run ids in run_patches_by_id MUST fail fast with diagnostics
**Reason**：per-run patch 参数已收敛为 `run_options_patches_by_run_id`；错误信息与场景描述需同步更新。
