...
- `scalim.yaml`（可选）提供 imports 治理配置：
  ...
  - 默认行为保持“未配置则不启用 roots 检查”（避免隐式改变既有项目行为）；推荐在半可信输入/CI 场景显式启用
  - （推荐）提供显式 override（Python API 或 CLI 参数）：用于在 CI/容器中稳定指定 project config 位置，避免“向上查找目录结构”导致的隐式差异
...
- 本变更依赖 `yaml-path-escape-hardening` 提供的 allow-roots helper 与错误格式（roots/containment 校验必须复用同一 SSOT，避免两套策略漂移）。
