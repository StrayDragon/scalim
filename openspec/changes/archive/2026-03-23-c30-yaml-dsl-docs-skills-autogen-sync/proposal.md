...
- 建议在安全/语义变化类 changes（例如 `c2/c20/c25/c80`）落地前后尽早完成：越早把可复制片段改为注入与生成，越能避免“实现已变但文档/skill 漂移”的长期维护成本。
- 与 `just qa` 的 drift gate 强绑定：本变更的验收应以 `just gen-docs --check` / `just validate-agent-skill` / `just qa` 的 fail-fast 信号为主。
