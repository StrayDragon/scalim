...
这意味着“从当前 row value 继续向下取嵌套值”本质上是字段级读取问题,而不是源级整体结果 `normalize` 问题。当前因为 YAML 没有这层声明式能力,一旦下游 loader 返回 `{"CustomerMark": {...}}` 这种嵌套对象,用户就被迫写薄包装先拍平结果。
...
- 给源字段新增声明式 `extract` 配置,并将其作为唯一稳定的字段取值入口,用于从“当前 key 对应的 row value”读取值。
...
- 不在本变更中引入源级整体结果 `normalize`;该能力由 `yaml-source-normalize` 单独负责。
- 不在本变更中开放任意 Python 函数型 extractor.
- 不支持数组下标、通配符、JSONPath 或更通用的表达式语法;`[1]` 永远表示 “key=1”,不是 list index。
...
### 5) `output.fields` selector semantics stay unchanged
...
- 后续是否需要支持数组索引或转义点号,还是继续鼓励业务把这类结构保留在 wrapper/normalize?
