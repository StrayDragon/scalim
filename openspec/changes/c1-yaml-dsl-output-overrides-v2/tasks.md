...
- [ ] 1.1 在 `src/scalim/dsl/by_yaml/runtime/contracts.py` 中重设计 `RunOverrides` 以支持 `overrides.outputs`(YAML-shaped `list[dict]`;仅承诺 `name/container/fields`)
...
- [ ] 2.1 在 `src/scalim/dsl/by_yaml/schema_dsl/models/**` 增加顶层 `validate_unique_field_names`(boolean) 并定义 hover 文案(SSOT;说明仅在 `include_header: true`(显式或默认) 且 `header_fields_output_by: name` 时触发)
...
- [ ] 2.4 在 `src/scalim/dsl/by_yaml/config_parsing/validator.py` 实现“有效展示名”唯一性校验(默认启用;仅当 YAML `outputs` 使用 `include_header: true`(显式或默认) 且 `header_fields_output_by: name` 时触发;开关为 false 时跳过)
