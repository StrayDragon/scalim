...
在引入 `PreloadCache.get_or_load()` 的 inflight 去重（`c0-preload-cache-inflight-dedupe`）之后，“持锁执行用户 `load_fn`”的死锁风险可被消除，但仍存在一个现实问题：当 `load_fn` 因外部依赖（网络/I/O/第三方库）长时间阻塞时，同 key 的等待方会无限期挂起，CI/服务会表现为“偶发卡死”，且缺少明确诊断线索。
