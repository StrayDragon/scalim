...
- **WHEN** demand YAML 包含 `imports: {common: ./_fragments/common.yaml}`
...
系统 MUST 施加最大展开深度上限,超过上限 MUST 报错以避免病态递归。
...
### Requirement: Path aliases are expanded via Python run/compile options
系统 MUST 在 Python 运行入口 `scalim.dsl.by_yaml.run()` / `compile()` 提供可选参数 `path_aliases: Dict[str, str]`。
当路径字符串以以下前缀开头时,系统 MUST 按 `path_aliases` 展开为实际文件路径:
- `"@/..."` 使用 `path_aliases["@"]`
- `"ALIAS:/..."` 使用 `path_aliases["ALIAS"]`
若 alias 缺失,系统 MUST 报错。

#### Scenario: alias path is expanded
- **GIVEN** `path_aliases={"@": "/a/b/c", "COMMON": "/a/b/c/_fragments"}`
- **WHEN** `imports.common` 为 `"COMMON:/common.yaml"`
- **THEN** 系统加载的文件路径 MUST 为 `/a/b/c/_fragments/common.yaml`

### Requirement: Validation runs on expanded final config
系统 MUST 在执行 schema-only 校验与语义 validator 之前完成 import 展开,并在“最终配置”上进行校验与编译。
