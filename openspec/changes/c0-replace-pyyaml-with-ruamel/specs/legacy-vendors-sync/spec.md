...
#### Scenario: scalim YAML parsing uses a vendored repository-defined facade
- **GIVEN** `src/scalim/vendor/yamlx/` 内包含 vendors 化的 YAML 实现
- **WHEN** `scalim` 在运行时解析 YAML 文本
- **THEN** 系统 MUST 通过仓库定义的 vendored YAML facade/默认后端完成解析
- **AND** 该 facade MUST NOT 依赖 `require_optional_dependency("yaml")` 或任何外部安装包
- **AND** facade 的内部默认 backend MAY 由 vendored `PyYAML` 或 vendored `ruamel.yaml` 提供
