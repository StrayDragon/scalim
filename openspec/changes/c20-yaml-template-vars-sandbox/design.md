...
- 在 `Template._get_variable` 的属性访问/方法调用逻辑处引入 sandbox gate：
  ...
  - **无论 sandbox/legacy 模式**：
    - 对以下划线开头属性（含 `__dunder__`）的访问 MUST fail-fast（避免自省逃逸链路；不提供放宽开关）
- 该 gate 必须覆盖：
...
- 仅通过“净化 template_vars 为纯 JSON-like”来间接 sandbox：不足以阻止对 `str/int/...` 的 `__class_`_ 自省链与 `__subclasses__()` 等经典逃逸路径（仍需要禁用 `_` 属性与方法调用）。
...
- 在 `maybe_precompile_yaml_text` 与高层入口（建议：`RunOptions`）增加显式开关：
  - `template_sandbox="safe"`（默认，安全）
  - `template_sandbox="legacy"`（显式 opt-in，不安全；允许**非 `_` 属性访问**与**无参方法调用**）
...
- LiteJinja2 的表达式支持与解析顺序应尽量对齐 Jinja2 的直觉（例如 dict key 优先、属性回退），但在 Scalim 中必须服从安全边界（默认 sandbox；`_` 属性永不放开）。
...
- 在进入渲染前递归校验 `template_vars`：
  - 仅允许：`None/bool/int/float/str/list/tuple/dict`（v1：dict key 必须为 `str`）
  - v1（窄版本，优先可预期与安全）：dict key MUST 为 `str`；发现其它类型 → fail-fast（错误信息包含类型名与“路径”，不包含值明细）
...
> 使用后者 另外我需要 litejinja2 表现的就像 jinja2 那样 只不过我们有scalim框架限制

- `template_vars` 护栏的 “dict key 归一化” 策略：强制 `str(key)` 还是仅允许 `str`？（倾向 `str(key)`，但错误信息需指向原始类型。）

> 先给用户一个窄版本：v1 要求 dict key 必须为 `str`（fail-fast + 明确路径/类型）。若未来确需放宽（例如允许 `int` key），用后置 change 再讨论 `str(key)` 的兼容与可预期性。
