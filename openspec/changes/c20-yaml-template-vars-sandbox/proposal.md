...
- 提供显式的“信任模式”开关（避免过度限制可信场景的高级用法）
  - 仅当调用方显式 opt-in 时，才允许“非 `_` 属性访问 + 无参方法调用”能力（`_`/`__` 属性仍默认禁止，避免自省逃逸链路）
...
- 建议在 `allowlist-footgun-hardening` 之后尽早落地：两者共同确立“默认安全 + 显式放宽 + 强告警”的一贯口径。
- 与 `yaml-dsl-docs-skills-autogen-sync` 互补：本变更新增/调整的参数与错误语义，后续应通过 docs/skill 的 SSOT 同步避免漂移。
