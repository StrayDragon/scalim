...
- [ ] 1.2 在 compiler 链路中实现 `$runtime.<name>` 占位符解析: 递归遍历 `main_source.params` 与 `sources.<id>.params`(dict/list),仅对 exact-match string 做替换.
...
- [ ] 2.1 扩展 `SourceIr` 以携带 `params: StaticParams`(与 `MainSourceIr.params` 对齐),并在 YAML `DemandConfig -> DemandIr` 转换时填充该字段.
- [ ] 2.2 更新 preload 执行路径: `Pipeline._preload_cached_sources()` 预加载调用透传 `source.params` kwargs(为空时保持零参调用),并确保 instrumentation 记录的 params 与真实调用一致.
