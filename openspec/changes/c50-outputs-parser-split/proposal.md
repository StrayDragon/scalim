...
- 建议在 SSOT 基建完成后再推进 staged 拆分：
  - `ordered-unique-ssot`（去重保序 SSOT）
  - `output-aggregate-producer-keys-ssot`（aggregate producer keys SSOT）
 这样可以避免“拆分中途还要搬常量/语义”的交叉改动，降低 review 与回归成本。
