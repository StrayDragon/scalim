...
- [ ] 1.3 确认 workflow lifecycle 的 SSOT 分层与 preflight 插入点：runtime policy merge 之后、engine execute 之前。
- [ ] 1.4 确认 preflight 的失败语义为 fail-fast + 直接 raise，整体 workflow 失败（不做 per-run 继续跑）。
...
- [ ] 3.3 列出“可推理子集”的 runtime-only diagnostics 清单，并确认 v1 仅覆盖 `validate_unique_field_names`，其余逐步纳入。
- [ ] 3.4 决定 `scalim-cli yaml-dsl validate`(workflow validate) 是否需要 policy-aware 参数化，或保持 authoring-only 语义并在文档中明确限制。
