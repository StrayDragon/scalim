...
- [ ] 1.1 扩展 workflow JSON Schema: `workflow.runs[*].depends_on`/`workflow.runs[*].init_vars`(与 `$ctx` 引用节点)并保持旧配置兼容
...
## 3. ctx Store & init_vars Injection

- [ ] 3.1 定义 ctx 的最小契约(自动暴露字段集合/JSON-like 约束/size guardrails)并在实现中统一校验
- [ ] 3.2 实现 `$ctx` 引用节点的解析与错误诊断(缺失 key/上游未成功/路径非法)
- [ ] 3.3 实现 run-scoped init_vars 与 Python 入口 `init_vars` 的合并规则(建议 run-scoped 覆盖同名 key)
- [ ] 3.4 将 demand 编译改为“run 就绪后再编译”,确保 `$init_var` 在编译期能拿到最终注入值

## 4. share_preload_cache Interactions

- [ ] 4.1 定案 `share_preload_cache` 与 `$ctx` init_vars 的组合语义(增量预检查 vs 约束组合),并同步到 delta spec/主规范
...
- [ ] 5.4 ctx → init_vars 注入集成测试(上游输出路径注入下游 loader params)
