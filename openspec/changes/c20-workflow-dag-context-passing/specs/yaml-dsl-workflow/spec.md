...
### Requirement: workflow provides run-scoped context and can inject it into downstream init_vars
...
- 系统 MUST 支持将选定的 ctx 值注入到下游 run 的 demand `init_vars` 中,以复用 demand 侧现有 `{$init_var: <name>}` 能力
- 初期实现 SHOULD 限定 ctx 值为 JSON-like(标量/小集合/映射),并对大体量数据提供 guardrails(作为后续扩展)

#### Scenario: downstream demand can use upstream exported ids via init_vars
- **GIVEN** run A 产出一组 ids 并将其写入 workflow ctx
- **AND** run B 依赖 run A
- **WHEN** workflow 执行 run B 且将 run A 的 ids 注入为 run B 的 `init_vars.user_ids`
- **THEN** run B 的 demand 配置中 `{$init_var: user_ids}` MUST 能解析到该 ids 集合
