...
覆盖面补充：
- `call_by` 不仅出现在 `fields.*.call_by`，也出现在 `outputs[*].aggregate.fields.*.call_by`（聚合后派生字段）。
- `call_by` head 引用既可能是 Python 引用（`pkg.mod:fn`），也可能是 builtin vocabulary（`^<id>`）；本变更关注 kwargs RHS 的字段 token，不改变 head 引用的既有语义能力。
...
- 覆盖 callsite：
  - `fields.*.call_by`
  - `outputs[*].aggregate.fields.*.call_by`（scope 优先 `aggregate.fields` out_field_id + `group_by`，并允许追加全局 field_id 作为低优先 fallback）
- completion 候选需要“分层 + 标注”，避免用户误以为所有候选都在运行时合法。
...
3) **completion 的候选来源与排序（按 callsite scope 分层）**
- `fields.*.call_by`：全局可见 field_id 为主集合。
- `outputs[*].aggregate.fields.*.call_by`：优先 `aggregate.fields` out_field_id，其次 `group_by` field_id，再追加全局 field_id 作为低优先 fallback（detail 标注来源与可能的语义不合法性）。
...
5) **解析策略：完整语法优先，输入中间态降级**
- 对合法 `call_by` 形态（括号闭合、kwargs 语法完整）优先使用轻量可诊断解析（尽量与 runtime `parse_call_by` 兼容的 tokenization 规则）。
- 对输入中间态（`x=`、括号未闭合、尾随逗号等）必须降级为“尽力抽取 + 空结果可诊断 warnings”，保证 editor 不失效。

## Risks / Trade-offs

- [误命中] 参数段内也可能出现字符串/数字等 → 仅把“未引用/未引号包裹的 identifier token”视为 field-id，其他类型直接降级为空 + warnings。
...
- [语义漂移] aggregate.call_by 的运行时允许集合更窄，但 completion 需要给全局 fallback → 通过分层排序+明确标注，降低误配风险；definition 仍是主要价值点。
