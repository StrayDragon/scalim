...
- 系统 MUST 支持 `yaml_dsl.import_aliases` 映射：
  - key 为 alias（例如 `@`）
  - value 为目录路径（建议相对 `scalim.yaml` 所在目录）
- 系统 MUST 在解析 `imports.<alias>` 路径时先应用 alias（例如 `@/x.yaml`），再执行后续的相对路径归一化与安全校验
- 系统 MAY 支持 `yaml_dsl.import_allowed_roots`（或等价字段）用于限制 imports 解析后的目标文件必须落在允许 roots 内；越界 MUST fail-fast

#### Scenario: alias @ maps to project root and imports can use @/x.yaml
- **GIVEN** 项目存在 `scalim.yaml` 且配置 `yaml_dsl.import_aliases: {"@": "./"}`
...
- **GIVEN** `scalim.yaml` 配置了 `yaml_dsl.import_allowed_roots`
