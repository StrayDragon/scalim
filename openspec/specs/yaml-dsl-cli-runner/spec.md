...
- workflow YAML 执行入口：`scalim.dsl.by_yaml.run_workflow(..., allowed_modules=..., ...)`
...
#### Scenario: CLI rejects demand/workflow execution subcommands
- **WHEN** 用户尝试执行 `scalim-cli yaml-dsl run <demand.yaml>` 或 `scalim-cli yaml-dsl workflow run <workflow.yaml>`
