...
## Related Concepts
- Output composition (多输出组合)
- Container outputs (容器型输出)
- Failure policy (输出失败策略)
- Meta/audit error handling (元数据/审计错误处理)
- Unified destination-binding model (统一目标绑定模型)
- Effective header-name validation (有效表头名验证)
