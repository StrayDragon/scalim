...
当前实现通过 `PerformanceObserver` 与 `RelationObserver`(Observer presets)提供可观测性入口;核心指标结构定义在 perf_metrics,实现位于 performance/relations presets.

## Related Concepts
- PerformanceObserver (performance.py)
- RelationObserver (relations.py)
- 结构化指标 (perf_metrics.py)
- PrettyLoggingObserver (logs.py)
- Observer 管理器 (manager.py)
...
该可观测性信号 MUST 满足:
- wants-gated:遵循 wants-gated/payload lazy 语义,未订阅时不得产生额外开销;
