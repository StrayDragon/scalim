...
**状态: ✅ 已实现**
## Purpose
为 `src/scalim/` 提供一个可 vendors 化、可审计的 dataclasses 能力入口,在保持 Python 3.6 运行时兼容的同时避免依赖外部 `dataclasses` backport,并避免包内绝对导入在多份包共存时混入错误实现。

## Related Code (as implemented)
- `src/scalim/vendor/dataclassesx/__init__.py`（facade：py>=3.7 使用标准库,py==3.6 使用 vendored backport）
- `src/scalim/vendor/dataclassesx/_backport.py`（vendored `dataclasses` backport）
- `src/scalim/vendor/README.md`（provenance 与更新策略入口）
- `pyproject.toml`（发行物不再声明 `dataclasses` 运行时依赖）
- `scripts/check-py36-typingext-docker.sh`（py36 干净环境 import smoke）
