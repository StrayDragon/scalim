...
## Purpose
为 YAML DSL 的下游集成提供稳定”工具/自省”公开入口，避免下游依赖 runtime 的内部实现模块路径。

## Related Concepts
- tools 模块 (yaml_dsl.tools)
- load_output_config
...
- **GIVEN** 一个合法的 demand YAML 文件路径
- **WHEN** 调用方执行 tools 模块的 load_output_config
...
系统 MUST 通过 tools 模块暴露相对引用基准推导能力。
...
- **WHEN** 调用方执行 tools 模块的 derive_base_module_path
- **THEN** 返回值 MUST 为字符串模块路径（允许为空字符串表示根包）

### Requirement: tools module MUST be a curated public module
系统 MUST 提供 tools 模块作为稳定公开模块，并将其纳入 curated public surface 回归门禁（导入 smoke + `__all__` 白名单断言）。
