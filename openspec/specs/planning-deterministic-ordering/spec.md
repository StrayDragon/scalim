...
系统 SHALL 在同层可执行节点间使用稳定 tie-break 规则.

#### Scenario: 不同 hash seed 计划一致
- **WHEN** 对同一 demand/targets 分别在不同 `PYTHONHASHSEED` 下构建计划
- **THEN** `field_order` 必须一致
- **AND** compute 算子序列必须一致

### Requirement: 拓扑排序输出稳定且具 tie-break 规则
系统 MUST 提供稳定的拓扑排序:在相同的 nodes/edges 输入下,`topological_sort` 输出顺序 MUST 可重复且不受 `PYTHONHASHSEED` 影响.
当同一层存在多个可输出节点时,系统 MUST 使用稳定 tie-break 规则(例如按节点 key 的稳定排序)来决定输出顺序.

#### Scenario: 同图多次排序结果一致
- **GIVEN** 相同的节点集合与依赖边集合
- **WHEN** 多次调用 `IMPL_ROOT.utils.graph.topological_sort(...)` 进行排序
- **THEN** 输出顺序必须一致
...
- **WHEN** 调用方从 `IMPL_ROOT.spec.ir.binding` 导入稳定排序 helper
- **THEN** `build_stable_lookup_key_list` 导入 MUST 成功
- **AND** `stable_lookup_keys_list` 导入 MUST 失败
