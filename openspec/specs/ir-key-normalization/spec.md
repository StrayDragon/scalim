...
## Purpose
`key_normalization` 提供一个运行期可控的”稳定字符串口径”键匹配策略,用于解决 relations/derived outputs 中跨来源类型不一致导致的 miss/分组拆分问题(例如 `1` 与 `”1”`). 该能力为 `EXPERIMENTAL`,默认关闭(`raw`).

## Related Concepts
- Key normalization modes (raw/auto_str/force_str)
- Relations lookup matching
- Derived outputs (group_by/dedup_by)
- String normalization function (auto_str_normalize)
- Cast precedence (lookup_cast/key.cast)
- Adaptive parallel execution
- Loader mapping key-space
## Requirements
...
该开关 MUST 可在所有公开入口启用并最终落到执行核心的运行期上下文.
...
当同时存在显式 cast 与 `key_normalization` 时,系统 MUST 按优先级确定最终 match key:
...
- `force_str`: 无论 candidate key 来源,都 MUST 应用字符串规范化
...
- 若 collision 对应的 value 全部深度相等,系统 MUST 保留任一值并继续(并发出一次 redacted 告警)
- 若 collision 对应的 value 存在差异,系统 MUST fail-fast(避免 silent 选择导致隐性错误)

并且:
- 告警/错误文案 MUST NOT 包含任何明细 key 值
- 告警/错误文案 SHOULD 包含 source 标识、`key_normalization` 模式、collision 计数等上下文信息
...
当 relations 实际采用字符串规范化口径进行匹配时,relations 查找 MUST 使用规范化后的 key 口径,且该口径 MUST 在下列边界保持一致:

- Loader 调用入参(key 绑定)
- Loader 返回 mapping 的命中
- 针对缓存源,系统 MUST 在”匹配点”使用与 lookup key 相同的 key 口径进行命中

注意:
- 当 `key_normalization=auto_str` 且存在显式 cast 时,系统 MUST 使用显式 cast 的 key 口径,不得额外做字符串规范化
...
当 `key_normalization` 与显式 cast 组合导致”预期 key 口径”与 loader mapping 的实际 key 口径不一致时,系统 MUST 提供可诊断的告警/错误,并满足:
...
- **AND** 存在显式 cast,使得最终候选 key 口径为非字符串(例如 `int`)
...
- derived outputs 的 key MUST 逐字段应用字符串规范化后再参与分组/去重
- 输出行中对应字段值 MUST 使用规范化后的值
...
- **AND** 输出行中该字段值 MUST 为规范化后的稳定字符串

#### Scenario: derived outputs dedup_by merges semantically equal keys when enabled
- **GIVEN** derived outputs 的 `dedup_by` 字段在不同输入行中分别为 `1` 与 `"1"`
- **WHEN** 用户启用 `key_normalization=auto_str` 或 `key_normalization=force_str`
...
- **WHEN** 系统在 adaptive 下创建 per-task 子运行时并执行任务
...
系统 MUST 提供一个公共的"去重保序"工具函数作为 SSOT,并要求相关模块复用该函数.
