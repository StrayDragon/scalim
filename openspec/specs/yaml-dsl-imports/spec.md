...
- `src/IMPL_ROOT/dsl/by_yaml/config_parsing/loader.py` (文件路径入口自动展开)
- `src/IMPL_ROOT/dsl/by_yaml/config_parsing/validator.py` (校验基于展开后的最终配置)
- `src/IMPL_ROOT/cli/yaml_dsl.py` (CLI validate/schema validate 行为)
- `src/IMPL_ROOT/dsl/by_yaml/schema/demand.gen.json` (schema 支持 imports/$import)
...
### Requirement: V1 only supports same-directory fragment paths
系统 MUST 在 V1 仅支持“同级目录导入”:
- `imports.*` 仅允许同级文件名(可选 `./` 前缀),例如 `common.yaml` / `./common.yaml`
- 系统 MUST 拒绝以下路径:
  - 绝对路径
  - 包含目录分隔符(`/` 或 `\\`)的路径(除 `./` 前缀外)
  - `../` 父目录路径
  - alias 前缀路径(例如 `@/x.yaml`, `COMMON:/x.yaml`)

#### Scenario: non-sibling import path is rejected
- **WHEN** `imports.common` 为 `../common.yaml` 或 `fragments/common.yaml`
- **THEN** 校验 MUST 失败并提示 V1 仅支持同级目录导入
