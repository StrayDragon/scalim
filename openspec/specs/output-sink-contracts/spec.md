...
## Related Concepts
- Sink 接口 (sinks/sink_base.py)
- CSV/Excel/Pandas sinks (sinks/sink_csv.py, sinks/sink_excel.py, sinks/sink_pandas.py)
- 可选依赖处理机制
...
系统 MUST 支持两种模式：
- **escape** (CSV 默认): 将疑似公式的字符串以文本形式写出（例如前缀转义），确保工具不会将其作为公式执行
