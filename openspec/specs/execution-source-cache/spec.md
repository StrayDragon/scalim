...
## Purpose
支持 `cache_mode=preload_forever` 的数据源在 pipeline 启动前预加载,结果写入 `ExecutionRuntime.preloaded_cache` 并在关联加载时复用;计划元数据记录已缓存的数据源。对于高频且内存占用小的映射表(如国家地区表、枚举常量映射表),作为全局一次性导入使用以加速获值速度。

## Related Concepts
- `ExecutionPlan.preload_sources` (planning/builder)
- `Pipeline._preload_cached_sources` (execution/pipeline/base)
- `PreloadCache.get_or_load` (execution/preload_cache)
- Cache hit path (execution/executor/operators/load_ref/loader)
- `WorkflowCacheEntrySignature` (签名对齐)

## Requirements

### Requirement: 预加载缓存模式
系统 SHALL 支持 source `cache_mode=preload_forever`,并在执行前预加载该数据源结果。系统 MUST 将 `source.cache_mode` 约束为显式枚举(当前仅允许 `none|preload_forever`),并在语义校验阶段拒绝未知值。
...
#### Scenario: 预加载缓存透传参数
- **WHEN** source 配置 `cache_mode=preload_forever` 且包含 `params`
- **THEN** pipeline 启动前应调用 loader 并透传 params 的 kwargs
...
### Requirement: preloaded_cache 并发边界与 key 空间明确
系统 MUST 明确 `ExecutionRuntime.preloaded_cache` 的 key 空间与并发边界:

- `preloaded_cache` 为可注入容器,生命周期由调用方决定;系统默认仅承诺 per-key `in-flight` 去重与结果复用(不承诺跨进程/跨不同 signature 的全局去重)
- 当多个并发执行单元共享同一个 `preloaded_cache` 时,容器 MUST 是线程安全的(推荐使用 `PreloadCache`)
- `PreloadCache.get_or_load(source_id, ...)` 的 key 为 `source_id`(不包含 loader/params/normalize signature)。跨不同 signature 复用同一 `source_id` 可能产生错误复用,其风险与责任边界 MUST 在文档中明确

#### Scenario: 并发调用者 per-key in-flight 去重
...
### Requirement: PreloadCache signature guardrail 可选启用
系统 MUST 为共享 `PreloadCache` 场景提供可选的 signature guardrail(默认关闭),用于检测同一 `source_id` 被不同 signature 复用的风险。

开启后系统 MUST:
- 支持至少两种策略: `error|warn`
- 当同一 `source_id` 的 signature digest 不一致时,按策略 fail-fast 或产生强告警
- 诊断信息 MUST 可用于定位与迁移(至少包含 `source_id`、两次 signature digest、以及迁移提示)

signature digest 的 SSOT MUST 与 `WorkflowCacheEntrySignature` 对齐,至少覆盖: `loader_ref`、渲染后的 `params`、`normalize`、`key`、`lookup_cast`。

#### Scenario: signature mismatch 在 error 模式下 fail-fast
...
#### Scenario: signature mismatch 在 warn 模式下告警并继续
- **GIVEN** 共享同一个 `PreloadCache` 且该 cache 已记录 `source_id="s1"` 的 signature digest 为 A
- **WHEN** 再次请求 `source_id="s1"` 且本次 signature digest 为 B(A!=B)
- **AND** 策略为 `warn`
- **THEN** 系统 MUST 产生强告警且继续执行

### Requirement: loader 幂等性期望
当系统允许出现并发或重复触发 preload load 时,系统 MUST 明确 loader 的幂等性期望与风险边界:

- loader 实现 SHOULD 尽量满足幂等性(重复调用产生等价结果)
- 系统 MUST 明确: 不应依赖"永不并发/永不重复调用"的隐式假设来保证正确性
- 若 loader 不可幂等(例如包含外部副作用),系统 MUST 明确提示风险

#### Scenario: preload_forever 文档说明幂等性期望
- **WHEN** 系统文档描述 `preload_forever` / `PreloadCache` 的并发边界
