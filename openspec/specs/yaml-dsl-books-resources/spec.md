...
当 workflow book 输出声明 `write_lock=true`（例如 `resources.books.<id>.write_lock` 或 `resources.books.<id>.export_xlsx.write_lock`）时，系统 MUST 在 publish（staged → final）边界对目标 `final_path` 进行跨进程互斥。
