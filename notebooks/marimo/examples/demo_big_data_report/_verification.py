...
class PythonOracle:
    ...
    def __init__(self) -> None:
        ...
        self.expected_results: List[Dict[str, Any]] = self._engine.build_all_expected()
        self.expected_by_pk: Dict[Any, Dict[str, Any]] = {r["order_id"]: r for r in self.expected_results if "order_id" in r}

    def get_load_stats(self) -> Dict[str, int]:
        return self._engine.get_load_stats()

    def build_report(self, target_fields: List[str]) -> List[Dict[str, Any]]:
        return [{k: v for k, v in row.items() if k in target_fields} for row in self.expected_results]

    def verify(
        self,
        scales_output: List[Dict[str, Any]],
        *,
        ...
        expected_by_pk = self.expected_by_pk
        actual_by_pk = {r["order_id"]: r for r in scales_output if "order_id" in r}
        ...
        if passed:
            ...
            if len(mismatches) > 5:
                lines.append("  ... 以及另外 {} 处".format(len(mismatches) - 5))
            summary = "\n".join(lines)

        return VerificationResult(
            passed=passed,
            total_rows=len(scales_output),
...
def verify_scalim_output(
    ...
    max_mismatches: int = 10,
    collect_field_stats: bool = True,
    oracle: Optional[PythonOracle] = None,
) -> VerificationResult:
    """验证 Scalim 输出结果
    ...
    if oracle is not None:
        return oracle.verify(
            ...
            collect_field_stats=collect_field_stats,
        )
    ...
    return VerificationResult(
        passed=passed, total_rows=len(scales_output), checked_rows=checked, mismatches=mismatches, summary=summary, field_stats=field_stats
    )
...
class DetailedVerification:
    ...
    def __init__(self, scales_output: List[Dict[str, Any]], fields_to_check: Optional[List[str]] = None, tolerance: float = 0.01) -> None:
        self.scales_output = scales_output
    ...
    def verify_row_count(self) -> Tuple[bool, int, int]:
        ...
        return len(expected) == len(self.scales_output), len(expected), len(self.scales_output)

    def verify_field(self, field_name: str) -> VerificationResult:
        return verify_scalim_output(self.scales_output, fields_to_check=[field_name], tolerance=self.tolerance, max_mismatches=20)
    ...
    def verify_relation_type(self, relation_type: str) -> VerificationResult:
        ...
        return verify_scalim_output(self.scales_output, fields_to_check=fields, tolerance=self.tolerance, max_mismatches=20)
    ...
    def run_full_verification(self) -> DetailedVerificationReport:
        ...
        full_result = verify_scalim_output(self.scales_output, fields_to_check=check_fields, tolerance=self.tolerance, max_mismatches=100)
...
def python_build_order_report(target_fields: List[str], oracle: Optional[PythonOracle] = None) -> List[Dict[str, Any]]:
    """纯 Python 实现订单报表构建
    ...
    """
    if oracle is not None:
        return oracle.build_report(target_fields)
...
def compare_rows_by_pk(
    ...
    *,
    ...
    left_pks = {pk for pk in left_by_pk if pk is not None}
    right_pks = {pk for pk in right_by_pk if pk is not None}
    if left_pks != right_pks:
        missing_left = sorted(right_pks - left_pks)
    ...
    for pk in sorted(left_pks):
        left = left_by_pk.get(pk, {})
        right = right_by_pk.get(pk, {})
        if fields is None:
            check_fields = sorted(set(left.keys()) | set(right.keys()))
        else:
            check_fields = fields
        for field_key in check_fields:
            if not _values_equal(left.get(field_key), right.get(field_key), tolerance):
                differences.append("PK={} 字段={!r}: {} != {}".format(pk, field_key, left.get(field_key), right.get(field_key)))
                if len(differences) >= max_differences:
                    return False, "发现 >= {} 处差异:\n{}".format(max_differences, "\n".join(differences))
...
def verify_order_by(scales_output: List[Dict[str, Any]], order_by: List[str]) -> OrderByVerificationResult:
    """验证输出顺序是否满足 order_by."""
    if not order_by:
        return OrderByVerificationResult(passed=True, order_by=order_by, message="order_by is empty")
    indices = list(range(len(scales_output)))
    for raw_key in reversed(order_by):
        ...
        def _sort_key(idx: int) -> Tuple[int, Any]:
            value = scales_output[idx].get(field_key)
            if value is None:
                return (1, 0)
            if descending:
    ...
    expected = indices
    actual = list(range(len(scales_output)))
    if expected == actual:
...
def run_parallel_comparison(target_fields: List[str], output_dir: str = "/tmp") -> FileComparisonResult:
    ...
    with InMemoryColumnSink(field_names=target_fields) as sink:
        ...
        scales_results = sink.get_rows()

    # 2. 纯 Python 执行
    python_results = python_build_order_report(target_fields)

    # 3. 导出 CSV
    scales_csv = os.path.join(output_dir, "scales_output.csv")
    python_csv = os.path.join(output_dir, "python_output.csv")

    export_to_csv(scales_results, scales_csv, target_fields)
    export_to_csv(python_results, python_csv, target_fields)

    # 4. 对比文件
    matched, diff_summary = compare_csv_files(scales_csv, python_csv)

    return FileComparisonResult(
        matched=matched, scales_file=scales_csv, python_file=python_csv, row_count=len(scales_results), diff_summary=diff_summary
    )
