...
| `sources.*.lookup_chunk_size` | `SourceIr.lookup_chunk_size` | 仅 keys 模式有效;省略/`0`/`None`=不分片(延迟通常最优);过小会线性放大 loader 调用次数;**不是并行开关** | 仅在下游有 payload/IN 上限时设置,并取最大安全值;片间并行见 Python `parallelize_lookup_chunks`（[0.10.0](../releases/0.10.0/)） |
