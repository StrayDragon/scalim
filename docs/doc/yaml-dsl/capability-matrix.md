...
| 文件写出布局（c30） | `DemandRunRuntimeOptions.output_write_layout` / `OutputWriteLayout` | 行流/列 HOLD/列 WINDOW 属写策略；换部署需改 → Python；YAML 禁止 | `OutputWriteLayout.ROW_STREAM\|COLUMN_HOLD\|COLUMN_WINDOW`；未设时由 `streaming`+`excel_column_residency` 推导；见 `excel-column-residency.md` / `runtime-output-write-layout` |
