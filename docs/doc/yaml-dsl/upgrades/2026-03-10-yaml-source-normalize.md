...
这次升级为 lookup `sources.*` 引入 source-level 的 `normalize`,用于在字段级 `extract` 之前对 loader 的整体返回值做一次 whole-result normalization。

- 新增 `sources.<id>.normalize`(显式拒绝 `main_source.normalize`)
- v1 支持 `normalize.kind: index_by_key`:把 `list[row]` 归一化为 `key -> row`
...
- 你不希望再写一个“把 list 转 dict”的薄 wrapper,只为了满足 DSL 的 lookup 形状
...
- `key_field` 必填
- `key_field` 必须与 `sources.<id>.key` 一致(v1 只支持单字段 key;复合键暂不支持 `index_by_key`)
- duplicate key 默认 fail-fast;如要覆盖必须显式 `on_conflict: first|last`
...
说明:
- `normalize` 先把整体结果变成 mapping
- `extract` 再在每个 lookup key 对应的单条 row 上取字段

## 常见报错

- `main_source.normalize`:
  - 报错: validator 会指出 `main_source.normalize` 不允许
...
- 复合键:
  ...
  - 修复: 保持 loader 返回 mapping,或在 Python wrapper 中完成复合键 indexing
- duplicate key:
