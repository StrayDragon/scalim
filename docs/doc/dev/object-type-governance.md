...
- 当前量级(完整扫描后,约数): **`block≈678` / `total≈686` / `allow=2`**(完整扫描补齐类方法后曾约 1016; batch1 用 `RuntimeValue`/`JsonLike` SSOT 收窄约 −72; batch2 清 `params_template`/`runtime_linking`/`execute*` 约 −102; batch3 清 `workflow_compile_*`/`yaml_load`/`resource_override`/`resources_base` 约 −80; batch4 清 `converters`/`outputs` parse/`derived_outputs`/`output_composition_yaml`/`snapshots`/`binding`/`workflow_compile_outputs` 约 −78).
