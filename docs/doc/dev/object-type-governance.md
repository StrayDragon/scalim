...
- 当前量级(完整扫描后,约数): **`block≈601` / `total≈609` / `allow=2`**(完整扫描补齐类方法后曾约 1016; batch1 用 `RuntimeValue`/`JsonLike` SSOT 收窄约 −72; batch2 清 `params_template`/`runtime_linking`/`execute*` 约 −102; batch3 清 `workflow_compile_*`/`yaml_load`/`resource_override`/`resources_base` 约 −80; batch4 清 `converters`/`outputs` parse/`derived_outputs`/`output_composition_yaml`/`snapshots`/`binding`/`workflow_compile_outputs` 约 −78; batch5 清 `template_precompile`/`validator`/`_parse`/`load`/`imports`/`conversion_lookup`/`submission_unit`/`contracts`/`resources_workbook` 约 −77).
