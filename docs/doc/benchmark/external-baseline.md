...
### 4.4 Python 3.6 最低兼容边界（0.10 历史；0.20 起 floor 为 3.10）

官方测量环境是 3.10；本节为 **0.10 线的历史测量**（0.10 曾支持 Python 3.6，0.20 起支持窗口收敛为 3.10+，见根 `ROADMAP.md`），保留用于历史行为对照：
...
- 结论（历史）：0.10 在其最低兼容 3.6 下的内存主张与正确性**不依赖高版本 Python**。0.20 起不再承诺 3.6。
