...
- Workflow bundle(多 run;含下钻):`/?bundle=.tmp/artifacts/scalim-viz/examples/demo_big_data_report/workflow-bundle-advanced/scalim-viz/bundle_manifest.json`
...
- workflow bundle(多 run)可使用:
  ...
  - 或用“选择目录(回放)”导入整个目录
- 本地生成 workflow bundle 样例目录:
  - `.tmp/artifacts/scalim-viz/examples/demo_big_data_report/workflow-bundle/`(在 UI 中选择该目录或其下的 `scalim-viz/` 目录均可)
  - `.tmp/artifacts/scalim-viz/examples/demo_big_data_report/workflow-bundle-advanced/`(含 `bundle_manifest.json` + report.xlsx/detail.csv/metrics.csv)
...
- 本地生成示例位于(默认;已被 `.gitignore` 忽略):
  - `.tmp/artifacts/scalim-viz/examples/demo_big_data_report/events-only/`
  - `.tmp/artifacts/scalim-viz/examples/demo_big_data_report/workflow-bundle/`
