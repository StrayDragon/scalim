<script lang="ts">
  ...
  $: label = data?.label ?? data?.target_id ?? "output";
  $: meta = data?.target_id ?? "";
  $: kind = data?.kind ?? "";
  $: status = data?.status ?? "";
  ...
  $: highlightClass = data?.highlighted ? "ring-1 ring-violet-200" : "";
  $: focusClass = data?.is_focus ? "ring-2 ring-blue-400" : "";
  $: activeClass = data?.active ? "border-violet-500" : "";
  $: sequenceClass = data?.sequence_hidden ? "opacity-0 scale-95 pointer-events-none" : "opacity-100";
  $: wrapperClass = `min-w-[160px] max-w-[240px] rounded-xl border bg-violet-50/80 px-2.5 py-2 shadow-sm flex flex-col gap-1 transition-all duration-200 cursor-pointer border-violet-200 hover:border-violet-300 ${sequenceClass} ${dimClass} ${highlightClass} ${focusClass} ${activeClass} ${
    selected ? "border-violet-400 ring-2 ring-violet-200" : ""
  }`;
  $: chipClass = "text-[9px] font-mono tracking-[0.14em] uppercase px-1.5 py-0.5 rounded-full border border-violet-300 bg-white/70 text-violet-700";
</script>
...
<div class={wrapperClass}>
  ...
  <div class="flex items-center justify-between gap-2">
    ...
    <span class={chipClass}>OUTPUT</span>
  </div>
  <div class="flex items-center flex-wrap gap-1.5 text-[11px] text-slate-500">
    <span class="font-mono">{meta}</span>
    {#if kind}
      <span class="text-[10px] px-1.5 py-0.5 rounded-full border border-slate-200 bg-white">{kind}</span>
    {/if}
    {#if data?.is_primary}
      <span class="text-[10px] px-1.5 py-0.5 rounded-full border border-emerald-300 bg-white text-emerald-700">
        primary
      </span>
    {/if}
    {#if lastEvent}
