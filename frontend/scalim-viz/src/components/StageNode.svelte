<script lang="ts">
  ...
  $: wrapperClass = `min-w-[170px] max-w-[240px] rounded-xl border border-dashed bg-slate-50/80 px-2.5 py-2 shadow-sm flex flex-col gap-1 transition-all duration-200 cursor-pointer border-slate-200 hover:border-blue-300 ${sequenceClass} ${focusClass} ${dimClass} ${highlightClass} ${focusNodeClass} ${activeClass} ${
    selected ? "border-blue-400 ring-2 ring-blue-200" : ""
  }`;
</script>

<div class={wrapperClass}>
  <Handle type="source" position={Position.Bottom} class="h-2 w-2 rounded-full border border-slate-200 bg-white" />
  <div class="flex items-center justify-between gap-2">
    ...
    <span class="text-[9px] font-mono tracking-[0.14em] uppercase px-1.5 py-0.5 rounded-full border border-slate-200 bg-white/70 text-slate-500">
      STAGE
    </span>
  </div>
  <div class="flex items-center flex-wrap gap-1.5 text-[11px] text-slate-500">
    <span class="font-mono">level {level}</span>
    {#if lastEvent}
