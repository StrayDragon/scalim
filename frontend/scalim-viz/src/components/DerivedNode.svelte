<script lang="ts">
  ...
  $: wrapperClass = `min-w-[160px] max-w-[220px] rounded-xl border bg-orange-50/80 px-2.5 py-2 shadow-sm flex flex-col gap-1 transition-all duration-200 cursor-pointer border-orange-200 hover:border-blue-300 ${sequenceClass} ${dimClass} ${highlightClass} ${focusClass} ${activeClass} ${
    selected ? "border-blue-400 ring-2 ring-blue-200" : ""
  }`;
  $: chipClass = "text-[9px] font-mono tracking-[0.14em] uppercase px-1.5 py-0.5 rounded-full border border-orange-300 bg-white/70 text-orange-700";
</script>
...
<div class={wrapperClass}>
  ...
  <div class="flex items-center justify-between gap-2">
    ...
    <span class={chipClass}>DERIVED</span>
  </div>
