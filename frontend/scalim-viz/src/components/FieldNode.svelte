<script lang="ts">
  ...
  $: label = data?.label ?? data?.field_key ?? "field";
  $: meta = data?.field_key ?? "";
  $: status = data?.status ?? "";
  ...
  $: wrapperClass = `min-w-[160px] max-w-[220px] rounded-xl border bg-blue-50/80 px-2.5 py-2 shadow-sm flex flex-col gap-1 transition-all duration-200 cursor-pointer border-blue-200 hover:border-blue-300 ${sequenceClass} ${dimClass} ${highlightClass} ${focusClass} ${activeClass} ${
    selected ? "border-blue-400 ring-2 ring-blue-200" : ""
  }`;
  $: chipClass = "text-[9px] font-mono tracking-[0.14em] uppercase px-1.5 py-0.5 rounded-full border border-blue-300 bg-white/70 text-blue-700";
</script>
...
<div class={wrapperClass}>
  ...
  <div class="flex items-center justify-between gap-2">
    ...
    <span class={chipClass}>FIELD</span>
  </div>
  <div class="flex items-center flex-wrap gap-1.5 text-[11px] text-slate-500">
    <span class="font-mono">{meta}</span>
    {#if data?.is_ref}
      <span class="text-[10px] px-1.5 py-0.5 rounded-full border border-amber-400 text-amber-600 bg-white">
        ref
      </span>
    {/if}
    {#if lastEvent}
