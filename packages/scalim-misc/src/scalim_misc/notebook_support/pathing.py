...
def find_repo_root(start: str | Path, *, markers: Iterable[str] = _DEFAULT_MARKERS) -> Path:
...
def ensure_repo_root_on_sys_path(start: str | Path, *, markers: Iterable[str] = _DEFAULT_MARKERS) -> Path:
...
def demo_big_data_report_dir(start: str | Path, *, repo_root: Path | None = None) -> Path:
    root = repo_root or find_repo_root(start)
    return root / "notebooks" / "marimo" / "demo_big_data_report"


def demo_big_data_report_declared_yaml_dsl_dir(start: str | Path, *, repo_root: Path | None = None) -> Path:
    return demo_big_data_report_dir(start, repo_root=repo_root) / "chapters_of_yaml_dsl" / "declared_yaml_dsl"


def demo_big_data_report_yaml_path(start: str | Path, *, repo_root: Path | None = None) -> Path:
    return demo_big_data_report_declared_yaml_dsl_dir(start, repo_root=repo_root) / "ecommerce_report.yaml"


def demo_big_data_report_workflow_yaml_path(start: str | Path, *, repo_root: Path | None = None) -> Path:
    return demo_big_data_report_declared_yaml_dsl_dir(start, repo_root=repo_root) / "workflow_fixture.yaml"


def demo_big_data_report_workflow_demo_yaml_path(start: str | Path, *, repo_root: Path | None = None) -> Path:
