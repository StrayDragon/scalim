...
def load_customers_by_ids(
    ...
    if max_batch is not None and len(id_list) > int(max_batch):
        msg = f"customer lookup batch too large: {len(id_list)} ids (max_batch={max_batch})"
...
def load_customers_catalog_alpha_by_ids(ids: Iterable[object] | None = None) -> dict[int, dict[str, object]]:
    """alpha 维表: 名称前缀 `Alpha-`;与 beta 共用 `source_id=customers` 时必须仍走本 loader."""

    return {
        customer_id: {"customer_id": customer_id, "customer_name": f"Alpha-{customer_id}"}
        for customer_id in ([] if ids is None else [int(item) for item in ids])
    }


def load_customers_catalog_beta_by_ids(ids: Iterable[object] | None = None) -> dict[int, dict[str, object]]:
    """beta 维表: 名称前缀 `Beta-`;键与 alpha 重叠时若串目录会拿到 `Alpha-*`."""

    return {
        customer_id: {"customer_id": customer_id, "customer_name": f"Beta-{customer_id}"}
        for customer_id in ([] if ids is None else [int(item) for item in ids])
    }
