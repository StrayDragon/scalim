...
# purpose: 定义 LSP 编辑器语义核心：project discovery、静态无副作用 diagnostics、Python 引用定位、光标位置抽取（call_by kwargs / compute 表达式 token），均不执行用户代码。 [rev:c25]
# scope: src/scalim/
功能: yaml-dsl-lsp-semantics-core
  ...
  @req:r626 @human
  场景: Editor semantics core MUST support extracting Python references by cursor position`
    - 系统 MUST 提供一个基于 `yaml_text + position` 的抽取能力，用于把编辑器光标映射到 YAML DSL 内的 Python 引用字段。 抽取结果 MUST 至少包含：
      ...
      - 失败时的可诊断 warnings（不得抛出未捕获异常） 支持字段集合 v1 MUST 至少覆盖：
      - `loader`
      - `call_by`
      - `retry.should_retry`（包含常见嵌套路径下的该字段） 当上述字段的 scalar 为 YAML block scalar（`|`/`>` 及其变体）且跨多行时：
      - 抽取 MUST 仍然可用
    ...
    那么 MUST 返回空结果
  @req:r673 @human
  场景: Editor semantics core MUST extract field-id tokens from `call_by` kwargs value articipants
    - 系统 MUST 扩展光标抽取能力，使其能在 `call_by` 的参数段（`(...)`）内识别 kwargs 的 `=` **右侧** field-id token，并用于 editor/LSP 语义能力。 覆盖 callsite 至少包括：
      - `fields.*.call_by`
      - `outputs[*].aggregate.fields.*.call_by`
      - builtin callable：`call_by: "^<id>(...)"`（head 为 builtin id） 抽取必须满足：
