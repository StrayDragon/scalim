...
# purpose: 定义 YAML DSL 编译前端的两个步骤：静态编译（只解析 YAML/AST、不导入用户代码、不依赖 allowlist）与运行时链接（resolve modules + 执行 allowlist 约束）。 [rev:c25]
# scope: src/scalim/
功能: yaml-dsl-compiler-frontend
  ...
  @req:r351 @human
  场景: Runtime linking (resolution) MUST be the only phase that imports modules and MUST
