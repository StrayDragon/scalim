...
# purpose: 定义 Hook/Observer/事件体系的统一边界:事件契约、分发路径、组件装配与高频路径性能语义. [rev:c25]
# scope: src/scalim/
功能: hooks-observability-structure
  ...
  场景: workflow attribution meta is injected incrementally and wants-gated
    - 系统 MUST 支持在 `Event.meta` 中注入 workflow 归因字段:
      ...
      - `workflow_node_id`: 标识事件来自哪个 workflow 节点(对 demand 节点等于 workflow YAML 的 `runs[*].id`) 归因注入 MUST 通过"增量合并 meta"实现:
      - MUST NOT 改写既有 `Event.run_id` 语义(仍表示一次 demand 执行)
      - MUST NOT 改写既有 `Event.seq` 语义(仍由发送端在 `run_id` 内单调递增) 归因注入 MUST wants-gated:
