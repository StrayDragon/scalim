...
# purpose: 为项目级配置文件 `scalim.yaml` 提供自动生成的 JSON Schema，以获得与 demand/workflow 类似的编辑器补全与 schema-only 校验体验，并将其纳入同一条”SSOT → 生成物 → drift gate”的治理链路。 该 schema 仅描述 `scalim.yaml` 文件的结构，不改变 `scalim.yaml` 的可选性与 nearest-wins [rev:c25]
# scope: src/scalim/
...
功能: yaml-dsl-project-config-schema
  ...
  @req:r365 @human
  场景: `scalim.yaml` schema MUST validate the `yaml_dsl` section used by imports/discovery
