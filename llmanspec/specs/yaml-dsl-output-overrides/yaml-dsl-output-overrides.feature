...
# purpose: 为下游”UI 动态选字段/动态输出”场景提供单一标准做法：demand YAML 保持可复用（通常不声明 `outputs`），调用侧在 `run/compile` 时通过 typed `RunOverrides` 显式指定输出编排与 IO 覆盖。 [rev:c25]
# scope: src/scalim/
功能: yaml-dsl-output-overrides
  ...
  @req:r364 @human
  场景: legacy YAML-shaped overrides inputs MUST be rejected with actionable migration ints
  ...
  场景: YAML DSL runtime MUST accept typed `RunOverrides.outputs` (dataclasses)
    - 系统 MUST 在 YAML DSL runtime 的 `RunOverrides` 中提供 `outputs` 覆盖字段,用于在不修改 demand YAML 的前提下运行期指定输出编排。 与旧的 YAML-shaped `list[dict]` 不同，`RunOverrides.outputs` MUST 为 typed dataclasses 序列，并且 MUST 可从 YAML DSL runtime 模块稳定导入。 本 spec 仅承诺明细输出(detail)的最小子集,至少包含:
      - `name`
      - `fields`(有序 field_id list)
      - `to`(book/sheet 绑定) 或 `to.file`(csv 文件输出) 二选一
      - `write`(可选;仅在 book 绑定输出时允许) `RunOverrides.outputs[*]` MUST NOT 支持以下 keys/语义(未来如需支持应另开 change 增量扩展):
      - `where`
      - `from`
      - `aggregate`
  ...
  场景: RunOverrides.resources MUST remain IO-only for path identity; write uses policy API; budget removed
    - 系统 MUST 在 RunOverrides 中提供 IO-only 覆盖能力用于 resources/outputs_defaults path 等 identity/IO 声明:
      - overlay MUST 允许覆盖 books.*.path/allow_formulas 与 files.*.path/encoding 等 IO 字段
      - overlay MUST NOT 再把 write_defaults 作为主 SSOT 补丁路径（MUST 走 Python BookWritePolicy）
      - overlay MUST NOT 再接受 budget（能力已移除；传入 MUST fail-fast 并提示删除）
      - 若仍传入 write_defaults/budget overlay，系统 MUST fail-fast
      - overlay MUST NOT 以 xlsx_file/xlsx_memory 分支名作为稳定补丁 SSOT
      - overlay MUST NOT 覆盖输出定义层字段
  ...
  @req:r715 @human
  场景: demand compile and workflow compile MUST share the same overrides compilation peline
