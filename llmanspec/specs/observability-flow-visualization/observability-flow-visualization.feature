...
# purpose: 提供执行过程的可视化输出:VizGraphSnapshot 从 ExecutionPlan 生成 XYFlow 兼容的 nodes/edges 结构用于依赖图展示;VizEventStream 将 Hook 事件映射为可视化事件流支持离线回放. [rev:c25]
# scope: src/scalim/
功能: observability-flow-visualization

  @req:r61 @human
  场景: VizGraphSnapshot 生成与结构
    - 系统 SHALL 能够从 ExecutionPlan 生成用于可视化的 VizGraphSnapshot,并提供与 XYFlow 兼容的 nodes/edges 结构. VizGraphSnapshot MUST 为节点与边提供稳定的 `id`,以便与运行态事件流进行关联. 系统 SHALL 支持以下可视化结构:
  ...
  @req:r146 @human
  场景: 前端架构与展示
    - scalim Viz 前端 MUST 满足以下架构与展示要求: **架构分层**:
      ...
      - 顶层 App 壳 MUST 仅负责 provider 与布局组合,实际行为由独立面板组件负责 **展示要求**:
  ...
  @req:r168 @human
  场景: Workflow 回放支持
    - 前端 MUST 支持 workflow replay bundle 作为一等入口,并提供直观的层级导航与 drill-down 交互: **Workflow 入口**:
      ...
      - 支持 MUST 为增量扩展,不得破坏现有单 run replay 入口 **Drill-down 交互**:
      ...
      - workflow demand 节点 MUST 能一眼看出其 scope/语义,并提供可发现的 drill-down 动作 **导航可访问性**:
