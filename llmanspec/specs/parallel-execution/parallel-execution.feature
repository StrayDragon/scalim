...
# purpose: 定义执行层对外并发语义 `seq|adaptive`,以及 `adaptive` 下的调度边界、后端选择、结果提交与事件回放契约;并交叉引用同 LoadRef 内 chunk 并行层次（见 `execution-refloader-chunk-parallelism`）。 [rev:c25]
# scope: src/scalim/
功能: parallel-execution
  ...
  场景: adaptive runtime shared caches MUST document CPython+GIL-only safety
    - 系统 MUST 明确声明:`parallel_mode=adaptive` 并发路径中的共享 `dict/set` 缓存与计数器仅在 **GIL-backed CPython** 下承诺正确性(依赖实现细节而非语言语义保证)。 当 `parallel_mode=adaptive` 启用批次内并发时,执行层会在多个 worker 线程间共享部分 `dict/set` 缓存与计数器(例如 key normalize cache 与 load-ref cache)。系统 MUST 明确声明以下契约:
      - 这些结构当前不提供显式锁保护
      - 其并发正确性仅在 **GIL-backed CPython** 下成立
      - free-threaded/no-GIL Python 不在支持范围内(若要支持,必须引入锁或等价同步策略) 系统 MUST 在对应实现的模块/类级别或关键字段附近以 `NOTE:` / `WARN:` 注释形式写出上述信息,使维护者在阅读热点代码时能直接看到契约边界。
