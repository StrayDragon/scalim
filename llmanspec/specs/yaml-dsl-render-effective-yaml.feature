...
# purpose: 提供用于 review/debug/对拍的库侧 API，将”作者写的 demand YAML”渲染为 effective YAML（展开后的单文件等价配置），避免 imports/template 复用在 review 时变成黑盒。 [rev:c25]
# scope: src/scalim/
功能: yaml-dsl-render-effective-yaml

  @req:r125 @human
  场景: Library MUST render effective demand YAML by expanding template_vars and imports
    - 系统 MUST 提供一个用于 review/debug/对拍的**库侧 API**,将“作者写的 demand YAML”渲染为 effective YAML(展开后的单文件等价配置)。 该 API MUST 支持 `loads/dumps` 形态(或等价接口):
      - `load_effective_demand_yaml(<demand.yaml>, template_vars=...) -> mapping`
      - `dump_effective_demand_yaml(<mapping>) -> yaml_text` 渲染行为 MUST 至少包含:
      - 对 demand YAML 文本执行 template precompile(仅当调用方显式提供 template vars)
      - 对 demand YAML mapping 执行 imports/$import expansion 输出约束 MUST 满足:
