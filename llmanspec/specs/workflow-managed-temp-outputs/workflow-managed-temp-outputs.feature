...
# purpose: 允许 workflow 托管仅供写入节点消费的中间态 outputs，使用内存 artifact 而非强制落盘，并确保 typed artifact 在 pathless/pathful xlsx 写节点路径保留值域。 [rev:c999]
# scope: src/scalim/
功能: workflow-managed-temp-outputs
  ...
  场景: workflow MAY manage temp tabular outputs for write-node consumption
    - 当 demand outputs 仅用于 workflow 写入节点消费时,workflow MAY 托管这些 outputs 的中间态；当 workflow 选择托管时,系统 MUST 允许以实现细节选择合适的内存 artifact,而不是强制落盘。
      ...
      - 当下游 consumer 为 pathful 或 pathless xlsx 写节点时,系统 MUST 为对应 output 提供按 output 粒度的 typed in-memory artifact (InMemoryRows)
      - 对 pathful / pathless xlsx consumer 路径,系统 MUST 保留 FieldValue 值域,MUST NOT 强制经过 CSV 等价字符串化再做后置恢复
      - 若同一 output 同时被 CSV 等价 consumer 消费,系统 MAY 从 typed artifact 显式派生字符串 artifact,但 typed artifact MUST 是 xlsx spreadsheet book 路径的 SSOT
