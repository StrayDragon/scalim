...
# purpose: 定义 workflow YAML 的共享 `.xlsx` book 资源(以 `workflow.resources.books` 表达)的迁移约束与运行期契约: 确定性写入、冲突安全、可观测且可原子导出为最终 xlsx,并提供可稳定引用的内置 loader 供下游节点读取 sheet rows（不再提供 cell/sheet 预算护栏）. [rev:c999]
# scope: src/scalim/
功能: workflow-sheetbook-resources
  ...
  场景: pathless sheetbook MUST preserve typed internal rows for workflow reads
    - 系统 MUST 将 pathless xlsx / sheetbook 的内部 rows 定义为 typed internal rows,而不是 `CSV` 等价字符串 rows:
      - sheetbook internal baseline header MUST 继续使用 canonical field key
      - sheetbook internal rows/segments MUST 保留 `FieldValue` 值域
      - workflow-managed output 写入 pathless book 时,MUST NOT 先以字符串 rows 作为内部 SSOT
  ...
  场景: pathless spreadsheet serialization MUST happen only at final export boundary
    - 系统 MUST 将 pathless sheetbook 的 spreadsheet/export 转换限制在最终 commit/export 边界:
      - internal rows MUST NOT 因为未来可能导出 `.xlsx` 而提前统一字符串化
      - 最终写 workbook 时,MUST 仅对 `str` 应用 spreadsheet formula escaping 规则
  ...
  场景: pathless book MUST NOT enforce cell/sheet budget guards
    - 预算护栏口径以 `workflow-shared-output-containers` r27 为 SSOT（`BookBudgetPolicy` / `max_sheets` / `max_total_cells` 已移除，残留 `budget` MUST fail-fast）；本能力沿用该口径，其余 sheetbook typed/export/canonical-key 契约保持不变。
