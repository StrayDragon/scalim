...
功能: governance-readme-examples
  ...
  场景: README example SSOT is runnable in QA
    - 系统 MUST 经由仓库 examples gate（`just examples` 或等价）执行主线套件内 README 最小示例章节的 notebooks 侧 SSOT，并在 `just qa`（或等价 `check`）中强制覆盖；CI 固定小 scale 下 MUST 以退出码 0 表示章节跑通；失败时 MUST 非零退出并给出可定位摘要。专用 `readme-examples` 目标若保留，MUST 仅承担 drift/`--check` 或成为 examples 的薄别名，不得维持第二套执行真相。
    当 开发者运行 `just qa`
    那么 MUST 经由 examples gate（或等价）执行主线套件内的 README 最小示例章节，且在当前树默认 scale 下通过时整体不因此失败
    当 某一 README 最小示例章节抛错或断言失败
