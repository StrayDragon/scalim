...
# purpose: 定义 Workflow IR 作为 workflow 的统一底座,并将 workflow 的 authoring surface(例如 YAML)视为"编译到 IR 的语法前端",而不是直接驱动执行器分支逻辑. [rev:c25]
# scope: src/scalim/
...
功能: workflow-ir
  ...
  场景: WorkflowOptionsIr MUST carry resources_wait from YAML to runtime
    - 系统 MUST 扩展 workflow 编译产物中的 options(IR),确保 runtime 能消费 workflow-level 的资源等待与诊断策略. `resources_wait` 的配置来源 MUST 位于 runtime policy boundary（而不是 workflow YAML authoring surface）：
