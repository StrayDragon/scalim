...
# purpose: 定义 workflow 级 replay bundle 导出契约：包含 workflow scope 快照/事件流与 demand child replay 目录，支持 drill-down 引用，保持 child replay 现有协议不变。 [rev:c25]
# scope: src/scalim/
功能: workflow-replay-bundle
  ...
  场景: workflow snapshot MUST provide a workflow-first graph entry
    - `scalim-viz/workflow/viz_snapshot.json` MUST 提供 workflow 级图结构，用于作为 workflow-first 入口视图。 workflow snapshot MUST：
  ...
  场景: workflow snapshot MUST provide stable drill-down mapping via `demand_run_id`
    - 对每个可 drill-down 的 workflow demand node，workflow snapshot MUST 在 `node.data` 中携带：
      ...
      - `demand_run_id="<demand_run_id>"` 其中：
      - `demand_run_id` MUST 对应同 bundle 内的 demand replay run id（即目录 `scalim-viz/<demand_run_id>/`）
      - `demand_run_id` MUST NOT 为绝对路径 workflow snapshot MAY 提供 stages 信息用于 staged layout：
