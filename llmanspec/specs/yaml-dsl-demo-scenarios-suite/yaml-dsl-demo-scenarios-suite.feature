# language: zh-CN
# capability: yaml-dsl-demo-scenarios-suite
# purpose: 维护一组 YAML DSL 场景库（fixtures），覆盖电商/广告/客服三类域，并纳入自动化回归测试范围，确保 DSL 能力与实现持续对齐。 [scope-review-2026-07-13-c25-xlsx-ir-path-presence]
# scope: src/scalim/
...
功能: yaml-dsl-demo-scenarios-suite
  ...
  @req:r111 @human
  场景: ads-support-场景-yaml-存在
    - 必须成立：假如 场景库目录已初始化；当 维护者检查 ads 与 support 子目录；那么 每个目录 MUST 至少存在 1 个 `*.yaml` demand 文件
    假如 场景库目录已初始化
    当 维护者检查 ads 与 support 子目录
    那么 每个目录 MUST 至少存在 1 个 `*.yaml` demand 文件
  @req:r353 @human
  场景: 场景库-yaml-在-gate-中可校验通过
    - 必须成立：假如 场景库 YAML 已就位；当 开发者运行 examples gate；那么 runner MUST 执行对场景库 YAML 的校验/运行对拍
    假如 场景库 YAML 已就位
    当 开发者运行 examples gate
    那么 runner MUST 执行对场景库 YAML 的校验/运行对拍
  @req:r474 @human
  场景: coverage-matrix-文件存在
    - 必须成立：假如 场景库目录已初始化；当 维护者检查场景库根目录；那么 MUST 存在一份 coverage matrix 文件且内容可读
    假如 场景库目录已初始化
    当 维护者检查场景库根目录
