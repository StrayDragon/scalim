...
# purpose: 定义 hooks 和事件系统的事件类型和策略决策信号规范,包括 loader retry 事件和 policy decision signals。 [rev:c25]
# scope: src/scalim/
功能: hooks-events

  @req:r55 @human
  场景: 新增 `loader_retry` 事件用于观测重试尝试
    - 系统 SHALL 新增事件类型 `loader_retry`,用于表达"某次 loader 调用失败且系统决定按 policy 重试". 系统 MUST 在每次 retry runner 决定进入 sleep+下一次尝试之前发出该事件. 系统 MUST NOT 将每次可重试失败当作 `error` 事件;`error` 事件仅用于最终失败(不再重试)或不可重试错误. `loader_retry` payload MUST 至少包含:
  ...
  场景: runtime MUST emit policy decision signals that hooks can override (pre-run_ir)
    - 系统 MUST 支持一类 "policy decision signals"（不同于纯观测事件），用于在进入 `run_ir()` 之前允许 hook 改写候选 runtime policy 值。 v0 MUST 提供 `pre_use_batch_size` signal，并满足：
      ...
      - 结果注入：signal 分发完成后，系统 MUST 使用 decision 的最终 `value` 更新传给 engine 的 `ExecutionRequest.batch_size`。 **Note:** 为避免破坏既有 "订阅全部事件的 on_event hooks"，policy decision signals SHOULD 采用 typed dispatch（例如 `on_pre_use_batch_size`）或其它 opt-in 机制；不得默认广播给所有 `on_event` 订阅者。
