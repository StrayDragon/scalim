...
# purpose: 定义 demand/workflow 统一的 `resources.files` 文件输出资源入口,并约束 CSV 输出通过 `outputs[*].to.file` + `outputs[*].write` 绑定,取代 legacy `outputs[*].container`. [rev:c25]
# scope: src/scalim/
功能: yaml-dsl-file-resources

  @req:r113 @human
  场景: demand and workflow YAML MUST support `resources.files` as the unified file-output
    - 系统 MUST 提供 `resources.files` 作为非 book 文件输出的统一资源入口,并在 demand/workflow 两类 YAML 中保持一致:
      - demand: `resources.files.<file_id>`
      - workflow: `workflow.resources.files.<file_id>` 约束:
      ...
      - legacy `write_lock` 配置面 MUST 被移除；若用户仍提供该字段，系统 MUST fail-fast 并给出迁移提示 其中 `version_id` 取值约束：
      - standalone demand: `version_id` MUST 等于该 demand 的 `run_id`
      - workflow: `version_id` MUST 等于该 workflow 的 `workflow_exec_id` legacy `kind` discriminator MUST 被移除；若用户仍声明 `resources.files.<file_id>.kind`，系统 MUST fail-fast 并给出迁移提示（迁移到 `resources.files.<file_id>.csv_file: {...}` 分支写法）。 （demand-only）imports 支持：
  ...
  @req:r355 @human
  场景: temp-path creation for atomic file writes MUST mitigate TOCTOU in untrusted ut
