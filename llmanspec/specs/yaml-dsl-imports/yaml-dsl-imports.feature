...
# purpose: 为 demand YAML 提供跨文件复用能力: 顶层 `imports` + 的 `$import`(编译期展开),并在 schema/语义校验前完成展开. 说明: - `$import` 的允许范围以稳定 authoring surfaces 为准;详见 yaml-dsl-demand-imports-scope 规范. [rev:c25]
# scope: src/scalim/
功能: yaml-dsl-imports
  ...
  @req:r357 @human
  场景: imports path MUST support relative fragments (v2) without relying on git repo root
    - 系统 MUST 放宽 `imports.<alias>` 的路径规则,使其在 v2 支持相对路径 fragments,并且不依赖 git repo root 推断。 当解析 demand YAML 时(包含被导入的 fragments):
      - imports 路径解析基准 MUST 为**当前 YAML 文件**所在目录(确定性)
      - imports MUST 支持以下相对文件路径形态(`.yaml/.yml`):
      - `./x.yaml`、`x.yaml`
      - `x/y.yaml`(子目录)
      - `../x.yaml`(父目录) 同时,系统 MUST 拒绝以下路径:
