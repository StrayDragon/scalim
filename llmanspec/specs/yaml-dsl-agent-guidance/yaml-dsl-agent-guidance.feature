...
# purpose: 定义 `scalim-yaml-dsl` 手工维护 skill 的任务驱动组织方式,确保 agent 能基于最小入口、明确命令和按需 references 一次完成 YAML 编写、升级、校验、订正与渐进迁移方案设计. [rev:c25]
# scope: src/scalim/
功能: yaml-dsl-agent-guidance

  @req:r103 @human
  场景: Task-Driven Manual Skill Entry
    - 系统 MUST 提供手工维护的 YAML DSL skill 本体,用于把 agent 引导到正确的任务路径,而不是把 skill 退化为单个 schema 摘要页. 手工维护的 `SKILL.md` MUST 明确覆盖至少以下任务类型:
      ...
      - 为某类 legacy 批量报表脚本设计渐进迁移方案 `SKILL.md` MUST 指示 agent 先识别任务类型,再按需读取最少的 references,而不是默认加载全部参考资料. `SKILL.md` MUST 保持 routing-first: 只保留任务分流、最小命令入口与对一层直达 references 的链接; 详细场景预设、迁移 heuristics 与完整语法目录 MUST 放在被直接链接的 references 中.
  ...
  场景: CLI and LSP Guidance Is Explicit
    - 手工维护的 skill MUST 直接提供可复制的 CLI/LSP 指引,覆盖仓库内与脱离仓库两种使用方式。 同时，为避免命令文案随实现演进而漂移，该指引中的“可复制命令片段” MUST 通过 injected blocks 受控注入，并以 CLI parser 为 SSOT：
      - SSOT: CLI yaml_dsl 模块
      - skill 生成入口: agent skill 生成命令（注入 marker 内部内容；禁止手改区块内部）
      - docs-site 入口: 文档生成命令（对应 docs 页同样使用 injected blocks） 指引 MUST 明确包含:
      ...
      - `uv run scalim-cli yaml-dsl lint <paths...>`
      - `uvx scalim-cli yaml-dsl lint <paths...>`
      - `uv run scalim-cli yaml-dsl format <paths...>`
      - `uvx scalim-cli yaml-dsl format <paths...>`
      - `# yaml-language-server: $schema=...` 的 header 参考 skill MUST 明确指出 schema path 可通过 `scalim-cli yaml-dsl schema path` 查询,并提供 header 模板。 skill MUST 明确指出 canonical example 不应固化本机 `.venv/...`、`site-packages/...` 或仓库私有相对路径头部。
    当 用户在仓库内工作并请求校验 YAML
    那么 skill 必须提供 `uv run scalim-cli ...` 形式的命令
    当 用户不在仓库内但需要运行 CLI 校验
    那么 skill 必须提供 `uvx scalim-cli ...` 形式的命令
  ...
  @req:r467 @human
  场景: `scalim-yaml-dsl` skill MUST recommend plain scalars and multiline `call_by` horing
    - 当 skill/agent 输出或改写 YAML DSL 时，系统 MUST 将“可读性优先 + 工具保证一致性”作为默认风格： - 对 `loader/call_by/compute/retry.should_retry` 这类 string 值： - 在语义等价且不会触发 YAML 隐式类型的前提下，MUST 优先使用 plain scalar（不写无意义引号） - 当 `call_by` 很长或需要注释时，MUST 推荐使用 YAML block scalar（`|`）并按多行函数调用排版 - skill MUST 明确指出：团队可通过 `scalim-cli yaml-dsl format` 统一风格，并避免手工维护差异
  ...
  场景: Workflow CLI and LSP Guidance Is Explicit
    ...
    那么 skill 必须提供 `uv run scalim-cli yaml-dsl schema validate --schema src/scalim/dsl/yaml_dsl/schema/workflow.gen.json <workflow.yaml>` 形式的命令
    当 用户请求让编辑器对 workflow YAML 提供补全/hover
    那么 skill 必须给出 upsert 命令,至少包含: `uv run scalim-cli yaml-dsl upsert-lsp-comment --type workflow --comment-style all <paths...>`
