...
# purpose: 定义 `PreloadCache` 的完整行为契约：并发安全性、in-flight 去重、signature 冲突防护、诊断能力，以及并发场景下的幂等性期望。 [rev:c25]
# scope: src/scalim/
功能: execution-preload-cache

  @req:r42 @human
  场景: PreloadCache waiter path MUST read inflight and fallback state under the per-source
  ...
  @req:r45 @human
  场景: 预加载缓存模式
    - 系统 SHALL 支持 source `cache_mode=preload_forever`,并在执行前预加载该数据源结果。系统 MUST 将 `source.cache_mode` 约束为显式枚举(当前仅允许 `none|preload_forever`),并在语义校验阶段拒绝未知值。 预加载调用 MUST 与常规 loader 调用保持一致的参数语义:
