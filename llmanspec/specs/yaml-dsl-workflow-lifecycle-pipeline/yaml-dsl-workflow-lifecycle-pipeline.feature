...
# purpose: 定义 workflow 生命周期显式阶段管线（phase pipeline）：structural preload 保持 parser-only、overrides 合并、effective outputs/resources 驱动的 preflight diagnostics、确定性的 fail-fast 顺序。 [rev:c25]
# scope: src/scalim/
功能: yaml-dsl-workflow-lifecycle-pipeline

  @req:r131 @human
  场景: workflow runtime MUST implement a lifecycle pipeline with explicit phase
  ...
  @req:r373 @human
  场景: structural preload MUST remain parser-only and MUST NOT consume runtime-only diagnostics。
  ...
  @req:r492 @human
  场景: structural preload MUST reuse the same demand-YAML parse settings as runtime entrypoints
  ...
  @req:r571 @human
  场景: effective options/resources merge MUST be the earliest policy-aware boundary inference
  ...
  @req:r633 @human
  场景: inferable diagnostics trigger conditions MUST be evaluated using effective outputs/resources
  ...
  @req:r719 @human
  场景: preflight and runtime compile MUST share a SSOT helper for effective outputs/resources
