...
# purpose: 通过 dataclass 元数据生成 YAML DSL JSON Schema，作为校验与编辑器提示的唯一来源。 [rev:c25]
# scope: src/scalim/
功能: yaml-dsl-schema
  ...
  @req:r677 @human
  场景: YAML DSL JSON Schemas MUST allow YAML merge key (`<<`) where propertyNames is used
  ...
  场景: demand JSON Schema MUST encode composed outputs invariants
    - 系统 MUST 在 demand JSON Schema 中表达 outputs 的关键不变量，避免 schema validate 放行但 parser 失败： - `outputs[*].container` 已移除；出现时 MUST fail-fast 并给迁移指引（CSV → `resources.files` + `to.file`；xlsx → `resources.books` + `to.book`/`to.sheet`） - 当 output 未声明 `aggregate` 时（明细输出），系统 MUST 要求存在字段来源： - 显式提供非空 `fields`，或 - 通过 `from` 继承字段集合 - 为保持 `$import` 在编辑器侧可用，该约束 MUST 不阻断仅声明 `$import` 的 output_target
  ...
  场景: output.fields 解析与 schema 指引
    - 系统 SHALL 仅接受 `output.fields` 条目为对象（dict/alias），并支持以下解析方式；拒绝字符串、单键映射与签名匹配形式。 支持的条目类型：
      ...
      - 对歧义场景系统 MUST 报错并提示补充 `source` 或改用 alias/`field_id`。 显式条目的覆写项为除选择器键之外的其它键，用于覆写字段定义（例如 `name`/`relation`/`value_cast` 等）。 schema 的 `description`/`markdownDescription`/示例 MUST 展示 alias 复用与显式选择器的用法，并明确说明 YAML merge（`<<`）会丢失对象身份，merge 产物需要选择器键才能解析。
  ...
  场景: retry 字段纳入 JSON Schema 与 hover 指引
    - 系统 SHALL 在生成的 YAML DSL JSON Schema 中新增 retry 字段支持：
      ...
      - `_templates.retry` 对象映射（template_name -> retry policy 对象） Schema MUST 为 retry policy 字段提供：
      - 类型约束（bool/int/number/enum）
      - 范围约束（至少包含硬上限）
      - 简短 hover 指引，说明安全默认值与"默认不启用"的语义 约束：
  ...
  场景: schema 说明源代码级 `normalize` 及其执行顺序
    - 系统 MUST 在 YAML DSL JSON Schema 的 `sources.*` 定义中新增 `normalize` 字段，并在 `description` / `markdownDescription` 中明确说明：
      ...
      - `normalize.index_by_key` 的输入输出形状示例 约束：
  ...
  场景: `outputs.*.fields` 支持 YAML alias 与 object 条目
    - 系统 MUST 允许 `outputs[*].fields` 的每一项为以下两种之一：
      ...
      - YAML alias（object）条目：条目为某个"已定义字段对象"的 alias（展开后为 dict），系统 MUST 将其解析为该字段对象对应的 `field_id` 字段对象的来源包括：
      - `main_source.fields.*`
      - `sources.*.fields.*`
      - 顶层派生字段 `fields.*` schema MUST 在生成的 YAML DSL JSON Schema 中允许 `outputs[*].fields.items` 为 `string | object`，以避免 schema-only 校验与编辑器提示拦截 alias 写法。 当 object 条目无法通过"对象身份"（identity）反查到字段对象时，系统 SHALL 允许基于内容相等做兜底匹配，但仅当匹配结果唯一时才允许成功解析。
    假如 字段对象已定义
