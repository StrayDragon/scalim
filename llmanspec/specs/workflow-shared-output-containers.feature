...
# purpose: 定义 workflow 共享输出容器的资源声明、写入节点、确定性顺序、追加/合并语义、原子提交、可观测性及并发安全契约。 [rev:c25]
# scope: src/scalim/
...
功能: workflow-shared-output-containers
  ...
  场景: workflow YAML exposes a stable authoring surface for shared resources and write
    ...
    - 系统 MUST 从每个 run 引用的 demand YAML 中读取 `outputs[*].to` / `outputs[*].write` 推导等价的写入节点集合 迁移约束(破坏性变更):
  ...
  @req:r741 @human
  场景: shared resource plan creation MUST be atomic and joinable within a workflow ution
  ...
  场景: joinable get-or-create 的等待诊断
    - 系统 SHALL 为共享资源的 joinable get-or-create 提供可选的 wait diagnostics,使 waiter 等待过程可观测且可定位. 该诊断配置 MUST 作为 workflow-level SSOT 暴露(例如 `workflow.options.resources_wait`),并贯穿 YAML→IR→runtime. 约束:
  ...
  场景: workflow workbook exports MUST escape Excel formulas by default
    ...
    那么 schema-only 校验 MUST 通过
  @req:r223 @human
  场景: workflow MUST precheck Excel output-path collisions across books deterministically
  ...
  场景: shared outputs MUST commit into staging and publish on success
    ...
    - failure: 保留 staging（便于排障） staging 路径布局约束:
  ...
  @req:r385 @human
  场景: workflow shared resources MUST publish into versioned output roots and update nifest
  ...
  场景: shared book pipeline MUST release demand artifacts and plan segments after final consumers
    - 在保持成功后统一 commit/失败 discard 的前提下,系统 MUST 降低共享 book 路径的不必要内存双驻留:
    - 对 managed tabular artifact 的消费者闭包仅包含仍待执行且输入解析到该 artifact 的 workflow write nodes
    - book_sheet_rows(或等价)读取的是 book plan 而非 demand artifact;其可见性 MUST 由 plan 生命周期保证(不得在 commit/discard 前清空 plan)
    - 不计入 workflow 结束后的用户侧 Python 引用与非 workflow-managed 捕获
    - 当 write node 已成功将 artifact 物化进 book plan且该 artifact 已无剩余 write consumers 时,系统 MUST 释放 demand 侧内存副本
    - commit_all 成功或 discard_all 完成之后,系统 MUST 释放 plan 持有的 segment 行数据(清空 segments 和/或丢弃 plan)
    - 释放 MUST NOT 破坏 book_sheet_rows 基于 plan 的可见性语义
    - 释放点 MUST 通过既有 diagnostics/结构化日志可诊断(原因枚举至少含 no_remaining_consumers|commit|discard);MUST NOT 引入 YAML 开关;MUST NOT 以本需求强制新增公开 Event 类型
