...
# purpose: 明确输出资源的四层边界：resources 声明、write_defaults 策略、outputs 内容编排、runtime output extras（meta/audit），并将 write policy 和 extras 迁出 YAML 主线。 [rev:c25]
# scope: src/scalim/
功能: yaml-dsl-write-policy-and-output-extras
  ...
  场景: workbook write policy MUST use Python BookWritePolicy as the single SSOT
    - workbook 写策略 MUST 以 Python typed BookWritePolicy（或 ResourcesPolicy.books.*.write）为单一 SSOT:
      - workbook 级写入行为（mode/align_by/header_policy/on_mismatch/on_conflict）MUST 由 Python runtime policy 表达
      - YAML resources.books.*.write_defaults MUST NOT 再作为 authoring SSOT（出现 MUST fail-fast）
      - outputs[*].write MUST 继续仅承载 output-local 的最小展示/表头 override
  ...
  @req:r493 @human
  场景: `meta` and `audit` MUST be runtime output extras instead of YAML authoring s
