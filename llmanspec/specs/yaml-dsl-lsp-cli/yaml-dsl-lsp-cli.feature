...
# purpose: 提供一个跨编辑器可复用的 YAML DSL LSP server 启动入口（默认 stdio），并约束其日志/降级行为， 以保证编辑器集成稳定且可排障。 [rev:c25]
# scope: src/scalim/
功能: yaml-dsl-lsp-cli

  @req:r116 @human
  场景: CLI MUST provide a stable `scalim-yaml-dsl-lsp serve` entrypoint (stdio by default
  ...
  @req:r358 @human
  场景: serve initialization failure MUST be diagnosable and MUST NOT emit partial JSON-RPC
