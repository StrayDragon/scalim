...
# purpose: 允许 run_workflow 接受按 workflow run id 注入的 per-run runtime patches，支持覆盖 batch_size、parallel_mode、max_workers、demand_diagnostics、components 等参数，并确保安全边界参数不可覆盖。 [rev:c25]
# scope: src/scalim/
功能: workflow-run-patches
  ...
  @req:r740 @human
  场景: run_options_patches_by_run_id values MUST be typed patches (dict patches are allowed
    ...
    那么 `run_workflow` MUST fail-fast
  @req:r769 @human
  场景: unknown run ids in run_options_patches_by_run_id MUST fail fast with
