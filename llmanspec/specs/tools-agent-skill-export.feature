...
# purpose: 定义 `scalim-yaml-dsl` skill 自动生成器的职责边界，确保自动化只负责受控参考产物，同时保证输出可校验、可重建、不会覆盖手工维护的 skill 本体。 [rev:c25]
# scope: src/scalim/
功能: tools-agent-skill-export
  ...
  场景: Example Selection and Validation
    - 系统 MUST 仅导出一个 canonical full example,目标路径固定为 `agentdev/skills/scalim-yaml-dsl/references/generated/example-full/ecommerce_report.gen.yaml`. 该 canonical example MUST:
      ...
      - 在导出后通过 `scalim-cli yaml-dsl schema validate` 与 `scalim-cli yaml-dsl validate` 校验
  ...
  场景: Generated References Cover Workflow YAML
    - 系统 MUST 扩展 `scalim-yaml-dsl` skill 生成器,使受控生成 references 覆盖 workflow YAML 的语法与工具入口,并保持“schema/CLI/spec 为唯一真相”的导出策略. 至少 MUST 满足:
  ...
  @req:r768 @human
  场景: Generated syntax catalog MUST reflect workflow schema key paths (`books`, not legacy
