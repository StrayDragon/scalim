...
# purpose: 重构 workflow execute 模块结构（extract outcome_builder/scheduler_rules/resource_lifecycle/viz_reporter），不改变外部可观测行为、不增加热路径开销，并保持与项目 Python 支持窗口兼容(根 `ROADMAP.md`)。 [rev:c25]
# scope: src/scalim/
