...
# purpose: 定义 adaptive execution 的硬护栏：显式 `max_workers` 的 hard cap 与可选的任务等待超时（fail-fast 诊断）；chunk 并行复用本护栏的 W/`task_timeout_s`（见 `execution-refloader-chunk-parallelism`）。 [rev:c25]
# scope: src/scalim/
功能: execution-adaptive-guardrails
  ...
  @req:r276 @human
  场景: adaptive execution MUST support an optional task timeout for fail-fast diagnostics
