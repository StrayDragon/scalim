...
功能: yaml-dsl-runtime-policy-boundary
  ...
  场景: demand runtime-policy fields MUST move out of YAML mainline
    - demand 侧明显属于 runtime policy 的字段 MUST 从 YAML 主线迁出到 Python / CLI runtime entrypoints:
      ...
      - `validate_unique_field_names` MUST 迁出 YAML **Note:** 本要求约束的是 “runtime policy 的主线 authoring surface”。系统 MUST NOT 把 `batch_size` 等 runtime policy 重新引入 YAML authoring；若需要在运行时推导 derived 值,应通过 runtime entrypoints 的 typed surface（例如 hooks）实现,并保持调用方显式控制优先级最高。
  ...
  @req:r568 @human
  场景: extracted runtime policy MUST be controllable by runtime entrypoints and environment
  ...
  @req:r630 @human
  场景: workflow MUST run inferable runtime-only diagnostics at the effective-policy ndary
  ...
  @req:r676 @human
  场景: demand parsing MUST be parser-only; runtime-only diagnostics MUST run only at policy
  ...
  场景: sources lookup_chunk_size MUST leave YAML; LookupChunking is Python typed SSOT
    - 系统 MUST 将 keys 模式分片大小与片间并行从 YAML authoring 收口到 Python typed oneof（推荐名 `LookupChunking`）:
      - demand YAML MUST NOT 再接受 `sources.*.lookup_chunk_size` 作为主线 authoring 字段
      - YAML 出现该字段时 MUST fail-fast 并给出迁移到 `DemandRunRuntimeOptions.lookup_chunking`（或等价）的友好提示
      - 运行入口 MUST 提供 per-source `LookupChunking`：至少 `off()`（不分片）与 `sized(size=..., parallel=...)`（`size>=1`；`parallel` 仅允许出现在 sized 形态）
      - 未配置某 source 时 effective 分片 MUST 等价于 `off`
      - 片间并行 MUST 仅由 `sized(..., parallel=True)`（或等价）表达，且仍须满足 `execution-refloader-chunk-parallelism`（含 `parallel_mode=adaptive`）
      - 系统 MUST NOT 新增 YAML 并行键
      - 旧平铺 `parallelize_lookup_chunks` MUST 迁移到 sized 的 parallel 语义（兼容窗内 MAY 接受旧布尔并映射，但 MUST NOT 作为推荐 SSOT）
      - 本条与 demand 顶层已迁出的 `batch_size`（主行分批）正交
  ...
  场景: SourceCache and RowsReuse MAY stay in YAML with explicit Python override priority
    - 系统 MUST 允许 YAML 继续声明 `sources.*.cache_mode`（`none`/`preload_forever`）与 params 内 `$rows.cache_mode`（`batch`/`none`），并 MUST 在 Python `DemandRunRuntimeOptions` 提供 per-source typed 覆盖（推荐名 `SourceCache` 与 `RowsReuse`，类型名 MUST 拆开以免混称）:
      - 覆盖优先级 MUST 为：显式 Python 覆盖 > YAML 声明 > builtin 默认（source cache 默认 `none`；rows reuse 默认 `batch`）
      - 系统 MUST NOT 静默忽略 YAML 声明
      - `SourceCache` 与 `RowsReuse` MUST NOT 共用同一个平铺 `cache_mode` API 字段
      - workflow `cache_pool` MUST 继续只决定跨 run 共享；是否 preload 仍由 effective `SourceCache`/YAML `cache_mode` 决定
      - `RowsReuse` 的 effective `cache_mode` MUST 从 DemandIr.sources 目录解析并用于 LoadRef 分组/relation signature（见 ir-source-relations r694/r1110），MUST NOT 读字段图嵌套快照
  ...
  场景: csv encoding and output header defaults MUST stay documented and factory-aligned
    - 系统 MUST 保持下列默认并在工厂与 YAML 省略口径对齐:
      - `resources.files.*.csv_file.encoding` 省略时 effective MUST 为 `utf-8`（`DEFAULT_OUTPUT_ENCODING`）
      - `resources.books.*.xlsx.allow_formulas` 省略时 effective MUST 为 `true`（pathless book 仍 MUST 拒绝该字段）
      - `outputs[*].write.include_header` 省略时 effective MUST 为 `true`
      - `outputs[*].write.header_fields_output_by` 省略时 effective MUST 为 `name`
      - `RunOverrides` 标准工厂（含 `csv_file` / `xlsx_file_single_sheet` 等）的 `header_fields_output_by` 参数默认 MUST 为 `name`（与 YAML 省略一致）
      - IO 覆盖面（encoding / allow_formulas / OutputWriteOverride）MUST 继续可用
