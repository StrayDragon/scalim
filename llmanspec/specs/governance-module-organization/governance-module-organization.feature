...
# purpose: 定义运行时模块的边界、入口最小化与依赖约束,避免将内部实现路径误用为公共 API,并保持模块层级单向依赖与 Python 运行时兼容性(支持窗口与 floor 见根 ROADMAP,当前 3.10). [rev:c25]
# scope: src/scalim/
功能: governance-module-organization
  ...
  @req:r378 @human
  场景: workflow_compile hotspot MUST be decomposed into responsibility-focused submodules
  ...
  @req:r384 @human
  场景: output_composition hotspot MUST be decomposed into spec/router/builder submodules
  ...
  场景: openpyxl write helpers MUST use shared SSOT util
    - 系统 MUST 将 openpyxl write_only 关闭与原子保存辅助逻辑集中到单一内部 util(`src/scalim/_internal/utils/openpyxl_helpers.py`):
      - `best_effort_close_write_only_worksheet` / `best_effort_close_write_only_workbook_worksheets` / `save_openpyxl_workbook_atomic` MUST 为 SSOT
      - `workflow` 的 workbook/sheetbook 资源与 `sinks` Excel 实现 MUST 复用该 SSOT,不得再维护平行副本
      - util MUST NOT 反向依赖 `sinks`/`workflow` 领域模块
      - `resources_workbook` 对外 re-export 的兼容符号 MUST 保持可导入
