...
# purpose: 定义结构化日志的 JSONL 输出格式、字段元信息、归因上下文自动注入及 CLI 渲染能力，确保日志可被机器解析与人类友好展示。 [rev:c25]
# scope: src/scalim/
功能: structured-logging

  @req:r76 @human
  场景: when enabled, `scalim.*` logs MUST be emitted as JSONL (one JSON object per Lines
  ...
  @req:r443 @human
  场景: the system MUST provide key metadata with full name + unique abbreviation, and SSOT
  ...
  @req:r659 @human
  场景: enabling structured logging MUST be configurable via env and explicit Python
  ...
  @req:r701 @human
  场景: the system MUST be single-write when structured logging is enabled (no legacy line
  ...
  @req:r737 @human
  场景: `scalim-cli` SHALL provide human-friendly and llm-friendly renderers for structured
