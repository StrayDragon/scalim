...
# purpose: 定义 `scalim-cli yaml-dsl viz compile` 的静态 viz 产物导出契约：不执行 loader，按 demand / workflow 两类入口写出 `viz_snapshot.json` 与 `viz_schedule_plan.json`（workflow 另含按 `run_id` 的子产物与 bundle manifest）。 [rev:c25]
# scope: src/scalim/
