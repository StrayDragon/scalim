...
# purpose: 定义 YAML DSL runtime 作为 DSL adapter/编译器的边界与对外入口，明确 YAML 配置（outputs/可观测性/retry 等）在编译期映射为 DSL-agnostic 运行请求对象的规则。 [rev:c25]
# scope: src/scalim/
功能: dsl-runtime-structure
  ...
  场景: yaml_dsl runtime 是纯 adapter/编译器
    - 系统 MUST 将 yaml_dsl runtime 的职责收敛为 DSL adapter：
      ...
      - 将 execution core result 包装为 YAML wrapper result yaml_dsl runtime MUST NOT 直接承担执行编排主流程（plan 构建、engine 实例化/调用、sink finalize、observer manager 生命周期），这些 MUST 由 execution 的统一 IR 编排入口负责。 yaml_dsl runtime 同时 MUST NOT 承载 workflow 的执行编排；workflow runtime MUST 位于 framework 层，YAML workflow 入口仅做前端编译与依赖注入。
    当 审阅 YAML DSL 的运行路径
  ...
  场景: official facade MUST preserve current extension seams
    - 系统 MUST 保持当前已确认的受控扩展点继续可经由官方 facade 使用，而不是通过删减能力来完成"收敛"。 本轮至少包括：
      ...
      - `RunOverrides`：输出覆盖、viz 配置等 系统 MUST 保持多输出组合能力为"受控 authoring surface"：由 YAML `outputs/resources` 与 `RunOverrides` 的受控覆盖项表达；官方 facade 不新增通用注入面，避免公共 API 膨胀。
  ...
  场景: entrypoints MUST accept single options object
    - 系统 MUST 将 demand 和 workflow 的运行入口收敛为 options-object 形态：
      ...
      - workflow: `run_workflow(workflow_yaml_path, *, options: WorkflowRunOptions)` `DemandRunOptions` MUST 作为运行期 knobs 的唯一承载对象（安全边界、模板、并行、重试、护栏、overrides、输出/capture 等）。 `WorkflowRunOptions` MUST：
  ...
  场景: YAML 特定配置编译为 DSL-agnostic execution request
    - 系统 MUST 在 yaml_dsl runtime 编译期将 YAML 的特定配置（如 batch_size、retry、template_vars 等）编译为 DSL-agnostic 的 execution request 字段，并在执行期由 execution 层统一消费。 编译规则：
      ...
      - `template_vars`/`init_vars` MUST 在 YAML parse 前完成预编译，确保 `DemandIr` 内持有的静态 params 可包含初始化对象 当调用方在 runtime/driver 层显式注入策略时，其优先级 MUST 高于 YAML 中的配置（与 guardrails 的覆盖语义一致）。
