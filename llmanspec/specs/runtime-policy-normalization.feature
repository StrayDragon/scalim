...
# purpose: 定义运行期 policy 值的归一化契约：以 Enum 为封闭集合的 SSOT，state/serialization 边界编/解码为 builtin `str`，允许值集合从 Enum 单一派生。 [rev:c25]
# scope: src/scalim/

功能: runtime-policy-normalization

  @req:r73 @human
  场景: policy values MUST normalize to builtin str literals at state/serialization boundary
