...
# purpose: 定义 YAML DSL 中 source ref 字段的 relation miss 默认值机制。允许用户在关联查询未命中时使用有序的默认值替代 None，提高配置表达能力和错误容错性。 [rev:c25]
# scope: src/scalim/
功能: execution-ref-miss-default-cases
  ...
  场景: selected default value MUST go through value_cast before writeback
    ...
    那么 写回值 MUST 为整数 `0`
  @req:r506 @human
  场景: default call_by MUST only depend on pre-ref available fields (compile-time fail-fast
