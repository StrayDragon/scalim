...
# purpose: 在不要求业务改动的前提下，降低 execution 热路径（`compute` / `call_by` / `load_ref` / write-precompute / row-wise fusion）的 per-row 固定开销与中间驻留，并保持既有值语义、可观测性与低内存特性。 [rev:c25] [c10-write-precompute-derived-fields] [c20-compute-expr-rowwise-fusion]
# scope: src/scalim/
