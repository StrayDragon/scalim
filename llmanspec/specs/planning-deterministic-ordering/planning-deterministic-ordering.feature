...
# purpose: 定义 scalim 的稳定顺序最小契约，用于保证 planning/执行在相同输入下可复现，并避免 `PYTHONHASHSEED` 等非确定性因素影响结果。 本 spec 覆盖执行计划构建顺序、拓扑排序输出的 tie-break 规则、以及 keys 绑定列表的稳定顺序要求。 [rev:c25]
# scope: src/scalim/
