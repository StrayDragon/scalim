...
- [x] 1.1 切换 xlsx_file 到 ROWS + excel OutputSpec
- [x] 1.2 补充/更新编译侧单测（若有）
...
- [x] 2.1 跳过 ROWS 的 `to_csv_artifact()`
- [x] 2.2 回归：仅 CSV plan 仍产出 csv map
- [x] 2.3 确认 `in_memory_rows_to_in_memory_csv` 仍可从 `scalim.sinks.rows` 导入（显式工具保留，collect 不调用）
...
- [x] 3.1 write_sheet / append_sheet book 分支
- [x] 3.2 legacy workbook 分支改 tabular
- [x] 3.3 helper + 单测
...
- [x] 4.1 重写 segment / sheet plan 模型
- [x] 4.2 apply_workbook_sheet 物化路径
- [x] 4.3 apply_workbook_append 物化路径（含 mismatch）
- [x] 4.4 commit 写出 typed rows
- [x] 4.5 更新 `tests/workflow/test_workflow_resources_coverage.py` 等直接测 workbook 的用例
...
- [x] 5.1 放开 apply_book_* 对 Rows
- [x] 5.2 iter_book_sheet_rows 支持 xlsx_file
- [x] 5.3 单测：xlsx_file book_sheet_rows + 可见性

## 6. 集成 / MVP / 门禁

- [x] 6.1 `uv run python3 llmanspec/changes/c5-xlsx-file-numeric-type-loss/examples/numeric-type-loss/run.py` — 场景 A/B 的 xlsx_file 类型丢失为 0
- [x] 6.2 `uv run pytest -x -q tests/workflow tests/yaml_dsl`（或全量 `tests/`）
- [x] 6.3 `llman sdd validate c5-xlsx-file-numeric-type-loss --strict --no-interactive`
- [x] 6.4 `just test-gate` + 相关 lint 已通过（100% core coverage / 3441 passed）；全量 `just qa` 另受既有 frontend npm advisory 阻塞，与本 change 无关
