...
| D3 `c30-output-write-layout-python-policy` | 闭集 Python 写出布局策略面 | apply 完成（≤5% 交错多跑 PASS） |
| D4 `c40-output-write-layout-advisory` | run_stats 建议，默认不改行为 | 设计稿（信息完备性+L1 schema+双跑实验） |
