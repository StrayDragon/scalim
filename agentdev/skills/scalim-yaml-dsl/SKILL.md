...
- **图边只存 source_id（c50）**: overlay 只写 `DemandIr.sources`；手写 IR 用 `to_source_id`/`source_id` + 显式目录。upgrade：`references/upgrades/2026-08-18-source-id-graph-refs.md`
...
- **MUST NOT** 为降低 Excel 峰值在 YAML 发明 `write.streaming` / books streaming knobs / `output_write_layout`;YAML Excel 组合层已是行 sink。列式调优用 Python **`OutputWriteLayout.COLUMN_CHUNKED`**（迁移窗：`ExcelColumnResidency.CHUNKED`；见 `references/streaming-column-excel-guidance.md` 与站点 `docs/doc/getting-started/excel-column-residency.md`）
