...
- YAML `resources.books` 只声明 identity（推荐 `xlsx`：有 `path`=落盘，无 `path`=内存总线；旧 `xlsx_file`/`xlsx_memory` 为 deprecated 别名）;`write_defaults` 与 `budget` 已迁出,出现即 fail-fast → 用 `WorkflowRunOptions`/`DemandRunOptions.resources_policy`(`BookWritePolicy`/`BookBudgetPolicy`,StrEnum 严格 in);详见 `references/upgrades/2026-07-12-book-write-policy-python-ssot.md` 与 `references/upgrades/2026-07-13-unified-xlsx-book-kind.md`
