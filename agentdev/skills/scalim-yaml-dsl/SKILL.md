...
- 需要阅读完整升级指南(SSOT): 读 `references/upgrades/*.md`(book write: `2026-07-12-book-write-policy-python-ssot.md`；book budget 移除: `2026-07-28-remove-book-budget-policy.md`)
...
- YAML `resources.books` 只声明 identity（唯一分支 `xlsx`：有 `path`=落盘，无 `path`=内存总线；`xlsx_file`/`xlsx_memory` 已硬删）;`write_defaults` 已迁出 → 用 `WorkflowRunOptions`/`DemandRunOptions.resources_policy`(`BookWritePolicy`,StrEnum 严格 in)；`budget` / `BookBudgetPolicy` **已移除**（YAML/`RunOverrides` 残留仍 fail-fast，删字段；内存交宿主限制）；详见 `references/upgrades/2026-07-12-book-write-policy-python-ssot.md`、`references/upgrades/2026-07-28-remove-book-budget-policy.md`、`references/upgrades/2026-07-13-unified-xlsx-book-kind.md`、`references/upgrades/2026-07-13-normalize-xlsx-book-ir-path-presence.md`、`references/upgrades/2026-07-20-remove-deprecated-xlsx-file-memory-kinds.md`
