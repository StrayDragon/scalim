...
llmanspec_requirement_map[15	]{slug	path	purpose	requirements}:
  yaml-dsl-schema	llmanspec/specs/yaml-dsl-schema/yaml-dsl-schema.feature	通过 dataclass 元数据生成 YAML DSL JSON Schema，作为校验与编辑器提示的唯一来源。 [scope-review-2026-07-13-c25-xlsx-ir-path-presence]	"YAML DSL JSON Schema generator MUST live in dev tooling and consume core SSOT\nschema generation entrypoint MUST remain single and output location MUST remain\nenums and defaults MUST be sourced from schema_dsl SSOT\nschema 生成器 MUST 支持文档标准化与 hover 指引\ndemand JSON Schema MUST validate source identifiers and reject empty loader/key\nYAML DSL JSON Schemas MUST allow YAML merge key (`<<`) where propertyNames is us\noutputs 字段 hover 指引明确可选与 overrides 推荐写法\nschema MUST expose the unified output target surface and reject legacy surfaces\ndemand JSON Schema MUST encode composed outputs invariants\n`header_fields_output_by` default is `name`\n顶层 schema 字段(guardrails)\n字段声明位置与 compute 约束\nschema documents `extract` as current-row-relative field extraction\nschema removes legacy `field` and provides migration guidance\nrelation steps-only 约束\noutput.fields 解析与 schema 指引\n字段 ID 唯一性与解析规则\n派生字段支持 call_by Schema\nschema meta key 参考文档与推荐写法\nschema meta 中 schema dict 不得吞掉 desc/md\nschema 明确 batch_size 的 null-or-int 语义\nretry 字段纳入 JSON Schema 与 hover 指引\ndemand JSON Schema MUST encode `lookup_cast` as a one-of cast-branch object\nschema 说明源代码级 `normalize` 及其执行顺序\n`outputs.*.fields` 支持 YAML alias 与 object 条目\nschema MUST support `{$init_var: <name>}` for resource paths\nkind-based `if/then` constraints MUST NOT trigger when `kind` is missing\nworkflow `resources` schema MUST expose only runtime-supported keys\ngenerated numeric constraints MUST declare numeric types\nschema/workflow drift gate MUST protect the highest-risk surfaces first\nschema MUST NOT expose runtime policy fields\nchanging-a-field-description-only-touches-core-ssot\nmissing-dev-package-does-not-break-runtime-imports\ndrift-gate-points-to-the-single-generator-entrypoint\nenum-drift-is-detected\n生成器产出正确的-schema-结构\nschema-hover-包含字段说明与示例\nschema-rejects-invalid-sources-configuration\nschema-validation-accepts-merge-key-in-map-like-objects\nschema-中包含-outputs-可选与-overrides-outputs-提示\nschema-exposes-unified-output-surfaces\nschema-rejects-legacy-output-surfaces\nschema-rejects-invalid-composed-outputs-configuration\nschema-default-for-header-fields-output-by-is-name\nguardrails-schema-校验\n顶层-fields-出现源字段\nschema-hover-包含-current-row-relative-说明\nschema-不再暴露-field\nrelation-id-字符串被拒绝\noutput-fields-解析支持显式选择器\noutput-fields-拒绝字符串与-merge-产物\n派生-源同名-field-id\ncall-by-仅允许派生字段且与-compute-互斥\ncall-by-语法说明可见且包含相对引用\n新同事查阅-schema-meta-key\nmain-source-order-by-hover-可见\nschema-batch-size-校验\nretry-schema-校验\nschema-rejects-legacy-lookup-cast-shape-with-name\nschema-rejects-sep-under-non-sep-first-branches\nschema-rejects-multiple-lookup-cast-branches\nschema-hover-包含-normalize-说明\nnormalize-约束校验\noutputs-fields-使用-alias-解析\nalias-identity-失败时-content-match-兜底\nschema-validate-不因-object-条目直接失败\nschema-validate-accepts-string-or-init-var-object-for-file-p\nschema-validates-import-based-mapping-without-false-positive\nschema-no-longer-exposes-runtime-policy-fields\nworkflow-resource-import-syntax-is-rejected-before-runtime\nschema-generation-fails-on-a-clearly-invalid-numeric-constra\na-drift-regression-is-caught-by-the-gate"
  demand-dsl	llmanspec/specs/demand-dsl/demand-dsl.feature	实现 YAML DSL 的加载、结构校验与 IR 转换流程,覆盖 main_source/sources/fields/relations 等配置,并在解析阶段使用安全 resolver 解析 loader 引用与 allowlist 限制,生成 DemandIr 供计划构建使用. [scope-review-2026-07-13-c25-xlsx-ir-path-presence]	"顶层结构与 IR 转换规则\nsource_id and sources keys MUST be valid identifiers and validated fail-fast\nunknown fields 诊断提供 suggestions(CLI/库一致)\nLoader 引用解析与 allowlist\ncallable preflight MUST run before building execution request\nSource/Bind 结构与 cache_mode/params（keys 分片迁出 YAML）\n使用语义清晰的配置 key 常量\nloader params templates support `{$runtime: <name>}` directives\n字段/关系表达式默认行为\nmain_source 批次排序配置\nbatch_size 语义、校验与一致性\nYAML DSL loader retry 配置\n`should_retry` 引用解析与 allowlist\n`normalize` 配置规则\naggregate 派生字段支持依赖驱动求值 (DAG)\naggregate fields 支持 safe compute 派生字段 (`compute`)\nsources-缺省视为\ndsl-version-被拒绝\nlegacy-字段被拒绝\nmain-source-与-sources-冲突\nguardrails-缺省\nguardrails-解析\ninvalid-source-id-is-rejected-early\nempty-loader-key-is-rejected-early\nroot-unknown-field-提示建议\nnested-unknown-field-提示建议\nloader-引用非法\n相对-loader-引用被归一化\n相对-loader-引用超出根层级被拒绝\nallowlist-missing-时拒绝解析\nclass-style-allowlist-精确到方法\npreflight-prevents-silent-compute-swallowing\nquiet-guardrails-does-not-suppress-preflight-errors\nlookup-chunk-yaml-rejected\nlookup-chunk-docs-recommend-max-safe\n解析器使用新常量\n旧常量不可导入\nmain-source-params-引用-init-var\n使用点号表达式定义-step\nextract-未声明时使用默认值\nlegacy-field-写法被拒绝\nstep-优先使用-field-id-映射\nstep-使用-data-key-时校验失败\nfield-id-与其他-data-key-重名时校验失败\nstep-引用未知字段校验失败\n多个-field-id-映射同一-data-key-不视为歧义\n解析-asc-desc-列表\n非法-order-by-条目被拒绝\n非主数据源字段被拒绝\n显式-null-被保留\n未声明-batch-size-使用默认\n非法值被一致拒绝\n顶层-retry-缺省\nanchor-merge-复用-retry-策略\nallowlist-缺失时-should-retry-被拒绝\n相对-should-retry-引用被归一化\nsources-normalize-通过校验\nmain-source-normalize-被拒绝\nrank-by-ratio-is-supported\npost-depends-on-post-is-supported\ncompute-ratio-then-rank-by-ratio-is-supported\ncompute-depends-on-post-is-supported"
  yaml-dsl-workflow	llmanspec/specs/yaml-dsl-workflow/yaml-dsl-workflow.feature	提供独立于 demand 的 workflow YAML,用于编排多个 demand 的批量执行,支持并发上限、失败策略与可选的 workflow-scope cache pool(用于共享 `preload_forever` 等缓存条目). [scope-review-2026-07-13-c25-xlsx-ir-path-presence]	"workflow public guidance MUST use curated stable entrypoints\nWorkflow YAML declares runs and options\nworkflow demand preloading MUST stay structural\nRuns execute demand YAML via existing compilation pipeline\nWorkflow enforces failure_policy\nworkflow preflight errors MUST be treated as workflow config/compile errors (ind\nmax_concurrency limits parallel runs deterministically\nworkflow nodes declare explicit DAG deps via `depends_on`\nworkflow provides a workflow-level ctx store (namespaced by `workflow_node_id`)\ndemand nodes MUST publish a minimal default ctx summary\n`$ctx` directives are resolved during compile-on-ready materialization\nfailure propagation cancels downstream nodes deterministically\nworkflow options expose a stable `cache_pool` configuration (replacing `share_pr\nworkflow entrypoints MUST be importable under the minimal supported environment\nworkflow emits workflow-level events and injects attribution for demand events\nmax_concurrency>1 requires thread-safe or stateless components\nworkflow YAML MUST use `workflow.resources.books` and MUST reject `writes` autho\nworkflow.options.resources_wait MUST configure join/wait diagnostics and timeout\nworkflow.options.output_staging MUST configure staging directory and cleanup pol\nworkflow scheduler preset MUST be configured through runtime entrypoints\nyaml_dsl public facade MUST export scheduler preset types under a stable import\n`run_workflow(...)` MUST orchestrate parse/preload/effective-merge/preflight via\nworkflow-examples-use-stable-facade-paths\nworkflow-file-passes-schema-validation\nresources-wait-in-workflow-options-is-rejected\nworkflow-ir-compile-accepts-duplicate-display-names-until-ru\ndemand-path-is-resolved-relative-to-workflow\nall-fail-stops-on-first-error\nprimary-only-continues-and-returns-errors\nprimary-only-does-not-continue-on-preflight-failure\nresults-preserve-declared-order\ndependent-nodes-start-after-prerequisites\ncycles-are-rejected-before-execution\nctx-is-only-readable-from-dependency-closure\ndownstream-can-consume-default-ctx-keys\nctx-driven-init-vars-trigger-compile-on-ready\ndownstream-nodes-are-cancelled-on-upstream-failure\ncache-pool-in-yaml-is-rejected-with-migration-guidance\nshare-preload-cache-is-rejected\nworkflow-entrypoints-imports-in-a-minimal-supported-environment\noptional-dependencies-remain-optional-for-core-imports\ndemand-events-can-be-joined-back-to-workflow-node-ids\nworkflow-level-events-have-workflow-exec-id-run-partition\ndocumentation-makes-component-concurrency-contract-explicit\nworkflow-yaml-rejects-removed-writes-field\nresources-wait-in-yaml-is-rejected-with-an-actionable-migrat\noutput-staging-in-yaml-is-rejected-with-an-actionable-migrat\nscheduler-preset-is-configured-through-runtime-entrypoints-n\ncaller-can-import-scheduler-presets-from-yaml-dsl-facade\nworkflow-execution-always-runs-preflight-before-engine-sched"
  yaml-dsl-books-resources	llmanspec/specs/yaml-dsl-books-resources/yaml-dsl-books-resources.feature	定义 demand/workflow 统一的 `resources.books` Excel IO 资源入口,并约束 `outputs[*].to` / `outputs[*].write` 的 book 绑定与导出语义. [scope-review-2026-07-12-c5-openpyxl-helpers]	"books MUST declare exactly one xlsx identity branch\npathful xlsx.path MUST define file export semantics and path resolution\ndemand MUST bind outputs to books via `outputs[*].to.book` and `outputs[*].to.sh\nstandalone demand MUST fail-fast when a referenced book resource is missing\nworkflow MUST merge books from demand/workflow with deterministic precedence and\nworkflow book patches MUST be applied with strict contracts and consistent diagn\nbook write behavior MUST come from Python policy; outputs.write stays header-local\nExcel exports MUST escape formulas for pathful xlsx books\nbook_sheet_rows MUST support unified xlsx pathful and pathless books\n`.xlsx` outputs MUST use books binding; legacy workbook container surface MUST b\nbooks MUST support unified xlsx variant with optional path\nbook runtime identity MUST be pathful vs pathless after normalize\npathless xlsx MUST remain a supported in-memory shared-book pattern\nYAML MUST fail-fast on removed book kind aliases\naccepts-only-unified-xlsx-branch\nxlsx-path-book-requires-a-path-when-exporting\nsheet-defaults-to-output-name-and-is-validated\ninvalid-default-sheet-name-fails-fast\nbooks-output-can-override-header-source-at-output-local-writ\ninclude-header-is-rejected-for-append-mode-books-output\nmissing-book-id-fails-fast-with-actionable-hint\nworkflow-overrides-a-demand-book-path\nunknown-book-patch-key-fails-fast-with-a-precise-path\nnested-write-defaults-enum-validation-is-consistent\npathless-rejects-append-align-by-header\npathful-xlsx-respects-allow-formulas\nbook-sheet-rows-supports-pathful-and-pathless-xlsx\nlegacy-container-output-is-rejected-with-migration-hint\nunified-xlsx-remains-only-ssot\npathful-xlsx-normalizes-without-kind-shim\npathless-xlsx-normalizes-without-kind-shim\npathless-memory-bus-is-valid\nxlsx-file-alias-is-rejected\nxlsx-memory-alias-is-rejected\nxlsx-memory-export-alias-is-rejected"
  yaml-dsl-output-overrides	llmanspec/specs/yaml-dsl-output-overrides/yaml-dsl-output-overrides.feature	为下游”UI 动态选字段/动态输出”场景提供单一标准做法：demand YAML 保持可复用（通常不声明 `outputs`），调用侧在 `run/compile` 时通过 typed `RunOverrides` 显式指定输出编排与 IO 覆盖。 [scope-review-2026-07-13-c25-xlsx-ir-path-presence]	"RunOverrides factory methods MUST stay stable including xlsx_file_single_sheet\nlegacy YAML-shaped overrides inputs MUST be rejected with actionable migration h\n`RunOverrides.outputs` MUST take precedence over YAML `outputs`\n`RunOverrides.outputs` MUST compile through the same outputs pipeline\nYAML DSL runtime MUST accept typed `RunOverrides.outputs` (dataclasses)\nRunOverrides.resources MUST remain IO-only for path identity; write uses policy API; budget removed\ndemand compile and workflow compile MUST share the same overrides compilation pi\ninvalid overrides MUST raise ScalimWorkflowConfigError with stable path\nxlsx-file-single-sheet-factory-name-remains-stable\nfactory-header-fields-output-by-defaults-to-name\nlegacy-dict-overrides-fail-fast\noverrides-outputs-wins-over-yaml-outputs\ninvalid-typed-overrides-outputs-fails-fast-with-a-diagnosabl\noverrides-provides-a-single-book-output\noverrides-provides-a-single-csv-output\nbook-path-overlay-without-deprecated-kind-branch\noverride-validation-behavior-stays-consistent-across-entrypo\ninvalid-typed-overrides-fails-with-scalimworkflowconfigerror"
  ir-source-relations	llmanspec/specs/ir-source-relations/ir-source-relations.feature	使用 `relations.*.steps` 描述主数据源到目标数据源的有序等值关联链,支持单步/多步/多字段关联,并在关联查找前应用 `lookup_cast` 归一化,执行时保持 left join 语义. [scope-review-2026-07-13-c25-xlsx-ir-path-presence] [c50-source-id-graph-refs]	"steps 结构与 relation 解析/推断规则\nlookup_key 与 lookup_cast/诊断\nref loader params are expressed by target-source params templates\n`$rows` preserves rows barrier semantics for relations\npreload_forever sources reject `$keys/$rows` directives\nlegacy `to_bind` is rejected with a copy-pastable migration hint\n批次内 LoadRef 复用与分片语义\nref loader 依赖信号驱动稳定排序\n关联诊断基于样本值并支持复合键对比\n关联路径与比较输出可读且稳定\nrelation `from` MAY reference pre-relation derived fields on main_source side\n字段图只存 source_id 策略只住目录\n多字段-step\n路径歧义\nrelation-id-字符串被拒绝\n关联缺失-无-default\n关联缺失-有-default\nsteps-驱动-loader-顺序\nstep-级-lookup-cast\n浮点被拒绝\n多字段缺失\nrelation-ref-loader-通过-keys-注入-lookup-keys\nrelation-step-without-bind-to-bind-remains-valid\nrows-触发-rows-barrier\npreload-forever-params-模板包含-keys-被拒绝\nrelation-step-to-bind-use-keys-param-被拒绝并给迁移建议\n同-relation-多字段只执行一次\nrows-cache-mode-none-禁用复用\nlookup-chunking-sized-分片加载\n多级-relation-排序正确\n样本值一致不报类型不兼容\n空样本返回空结果提示\nbroadcast-constant-derived-key-can-be-used-as-relation-from\nderived-key-that-depends-on-ref-fields-is-rejected\npython-rows-reuse-none-disables-group\nstored-demand-ir-uses-source-ids\noverlay-replaces-catalog-not-graph"
  ir-field-compute	llmanspec/specs/ir-field-compute/ir-field-compute.feature	定义源字段 `value_cast` 转换与派生字段 `compute/call_by` 的解析、校验与执行行为,并规范派生字段依赖推导(拒绝显式 `depends_on`)与表达式安全策略. [scope-review-2026-07-13-c25-xlsx-ir-path-presence]	"字段值 value_cast\ncompute 识别/校验/安全约束\ncompute expression builtin calls MUST validate arity when signature is inspectab\ncompute 安全引擎 MUST provide a safe `dec(x)` decimal helper\ncompute 表达式允许使用 `Decimal(...)` 构造器\n依赖推导规则与无依赖拒绝\n派生字段执行与错误处理\ncompute 表达式预编译并复用执行\ncompute 编译缓存有上限(有界 LRU)\ncompute failures MUST NOT log raw expressions by default\ncompute audit callback MUST support redaction\ncompile cache operations MUST be safe under concurrent access\ncall_by 派生字段函数调用\nderived field call_by MUST validate argument binding at compile time when possib\ncall_by 上下文引用\ncompute sandbox rejection MUST include an actionable `call_by` migration hint\n应用字段-value-cast\nvalue-cast-decimal-基本转换\nvalue-cast-decimal-对-float-使用-str-转换\nvalue-cast-decimal-对空白字符串视为缺失\nvalue-cast-str-对-none-透传\nvalue-cast-int-对-none-透传\n缺省不转换\n同时声明-compute-与-call-by\n缺失-compute-与-call-by\ncompute-语法错误\ncompute-引用未知名称\ncompute-表达式支持一元-not\ncompute-超过资源上限被拒绝\ncompute-builtin-arity-mismatch-is-rejected-early\ndec-converts-finite-float-without-binary-expansion\ndec-rejects-invalid-decimal-text\ncompute-使用-decimal-字符串字面量\ncompute-使用-dec-helper\n常量表达式允许\n空依赖但含函数调用仍拒绝\n显式-depends-on-被拒绝\n常量表达式复用且逐行触发事件\n含函数调用或依赖字段不复用\ntop-level-compute-may-return-decimal\ntop-level-call-by-may-return-decimal\neval-使用-code-object\ncache-size-不超过上限\nerror-message-contains-expression-hash-instead-of-raw-expres\naudit-disabled-does-not-call-callback\nredacted-audit-logs-only-field-names\nfull-audit-requires-explicit-opt-in\nconcurrent-compile-does-not-crash\n基本-call-by-解析\n相对-call-by-引用被归一化\ncall-by-支持-kwargs\ncall-by-支持-python-字面量\n非-python-字面量被拒绝\nallowlist-缺失\nmultiline-call-by-with-comments-is-accepted\npositional-argument-to-keyword-only-signature-fails-fast\n合法-ctx-属性\n非法-ctx-属性\ndict-get-method-call-is-rejected-with-a-call-by-hint"
  execution-preload-cache	llmanspec/specs/execution-preload-cache/execution-preload-cache.feature	定义 `PreloadCache` 的完整行为契约：并发安全性、in-flight 去重、signature 冲突防护、诊断能力，以及并发场景下的幂等性期望。 [scope-review-2026-07-13-c25-xlsx-ir-path-presence]	"PreloadCache waiter path MUST read inflight and fallback state under the per-sou\npreload cache hit path MUST be thread-safe under concurrent writers\nPreloadCache mapping introspection MUST be safe under concurrent mutation\nPreloadCache MUST document its thread-safety contract\nPreloadCache concurrency MUST be covered by tests\npreload cache MUST dedupe inflight loads without holding locks during load_fn\nsignature mismatch MUST be detectable when guardrail enabled\ndefault behavior MUST remain unchanged when guardrail disabled\ninflight wait diagnostics MUST be opt-in and include stable fields\nconcurrency scenarios MUST be explicit and reproducible\nloader idempotency expectations MUST be explicit\n预加载缓存模式\n关联加载优先命中缓存\n计划元数据记录缓存源\npreload cache 存储 normalized 结果\nwaiter-observes-completed-load-under-lock\ncache-hit-does-not-raise-keyerror-under-concurrent-updates\nconcurrent-iteration-and-load\nmaintainers-understand-boundaries\nregression-guard-for-thread-safety-fixes\nconcurrent-callers-observe-one-load-and-the-same-cached-resu\nsignature-mismatch-fails-fast-in-error-mode\nsignature-mismatch-warns-in-warn-mode\nguardrail-disabled-keeps-legacy-semantics\nlong-inflight-wait-emits-warning-only-when-diagnostics-enabl\ntwo-concurrent-callers-trigger-at-most-one-in-flight-load-pe\npreload-forever-docs-mention-idempotency-expectations\n预加载缓存数据源\n预加载缓存透传参数\ncache-mode-拼写错误被拒绝\n关联命中缓存\n记录缓存源\npreload-缓存写入-normalized-mapping\ncache-hit-路径不重复看到-raw-list"
  workflow-cache-pool	llmanspec/specs/workflow-cache-pool/workflow-cache-pool.feature	"提供 workflow-scope 的缓存池(`cache_pool`),用于在同一次 workflow 执行内跨 nodes 复用可共享缓存条目(当前主要用于 `preload_forever` 结果),并通过 signature-based keys/冲突策略/生命周期(refcount+pin)/预算策略/观测事件/并发安全确保\"复用正确且可诊断\". [scope-review-2026-07-13-c25-xlsx-ir-path-presence]"	"workflow options expose a stable `cache_pool` configuration (replacing `share_pr\ncache-pool-config-in-yaml-is-rejected-with-migration-guidanc\ncache-pool-is-enabled-through-unlimited-runtime-preset\ncache-pool-is-enabled-through-bounded-runtime-preset\nbounded-preset-supports-pin-for-selected-logical-keys"
  workflow-observability-bridge	llmanspec/specs/workflow-observability-bridge/workflow-observability-bridge.feature	定义 workflow 运行上下文与既有 hooks/observers 事件流的桥接契约，使 demand 事件可稳定归因到 workflow 节点，并提供最小的 workflow-level 编排事件。 [scope-review-2026-07-13-c25-xlsx-ir-path-presence]	"workflow attributes demand events for stable DAG correlation\nworkflow provides workflow-level observability events\nworkflow preserves demand hooks/observers semantics\nworkflow event catalog is extensible for cache/resources\ndemand-events-can-be-joined-back-to-workflow-runs\ncancelled-nodes-are-observable\nper-run-event-streams-remain-isolated\nnew-workflow-level-events-remain-joinable"
  runtime-pruning	llmanspec/specs/runtime-pruning/runtime-pruning.feature	PlanBuilder 基于目标字段构建依赖图并裁剪 required_fields,生成仅包含必需字段的 ExecutionPlan;运行时在 BatchContext 中仅保留 required_fields,并在列式/流式写入与显式释放时触发 FieldSlimEvent 以降低内存占用. [scope-review-2026-07-13-c25-xlsx-ir-path-presence]	"依赖剪枝与计划元数据\nYAML 转换阶段按 output.fields 剪枝字段定义\n运行时字段保留与释放策略\n未配置输出字段\n剪枝非目标字段\n未引用字段不进入-ir\n非必需字段被忽略\n列写入后释放"
  execution-loader-retry	llmanspec/specs/execution-loader-retry/execution-loader-retry.feature	提供可配置的 loader retry policy 机制，在 loader 调用因瞬态错误失败时执行有限的自动重试，支持全局默认策略与 per-source 覆盖，并通过可配置的退避策略、次数上限和耗时上限防止无限重试。 [scope-review-2026-07-13-c25-xlsx-ir-path-presence]	"Loader retry policy 配置模型与默认值\nshould_retry 回调契约\nshould_retry callback signature MUST be prechecked when enabled\nRetry runner 语义(次数/耗时/退避)\n全局策略与 per-loader 覆盖的解析\niterable/generator 的重试边界\n默认关闭无行为变化\n超出硬上限被拒绝\nshould-retry-false-不重试\nkeyword-only-should-retry-is-rejected-early\nattempt-上限生效\n单个-loader-禁用重试\ngenerator-迭代抛错不重试"
  runtime-guardrails	llmanspec/specs/runtime-guardrails/runtime-guardrails.feature	定义运行期 guardrails 配置与执行契约,用于对 loader/relations/compute 等环节的契约违规、数据质量问题与异常处理提供可配置的 quiet/fast_fail 行为,并通过现有错误事件通道进行可观测记录. [scope-review-2026-07-13-c25-xlsx-ir-path-presence]	"Guardrails 配置与默认行为\nruntime guardrails MUST NOT swallow callable preflight failures\nquiet 模式违规记录\nLoader 结果结构护栏\nRowLike 字段提取语义(无歧义优先级)\n关键字段缺失护栏\n字段 extractor/value_cast/transform 异常护栏\n默认关闭\nfast-fail-模式终止\ncompute-quiet-mode-does-not-convert-preflight-failure-to-non\nquiet-模式记录并继续\nloader-返回非-mapping\nrow-data-为非-dict-的-mapping-也可提取字段-修复静默-none\nrow-data-为属性对象\nrow-data-为-duck-typed-getitem-对象\n关键字段缺失\nrequired-fields-支持-yaml-alias\n转换异常触发-guardrail"
  performance-observability	llmanspec/specs/performance-observability/performance-observability.feature	PerformanceObserver 在 pipeline/batch/loader 事件上收集耗时、loader 统计与吞吐量,并可选采样内存/CPU(psutil 可选);RelationObserver 收集关联命中率与类型不匹配诊断. [scope-review-2026-07-13-c25-xlsx-ir-path-presence]	"性能指标采集与结构化输出\nduration 统计使用单调时钟\n资源采样与报告输出\nruntime entrypoints 装配与独立开关\nRelationObserver 统计与报告\nadaptive 调度决策可观测性\n共享 observer 与 workflow 结论边界\n高影响 Relation/Perf 扩展警告\nwrite stage 真实 sink 归因\nwrite 与 loader/compute 双计规则\n基础耗时监控\n阶段耗时可重复\nduration-非负且稳定\npretty-logging-uses-event-duration\n内存监控-无-psutil\ncsv-输出缺少路径\n批次耗时超阈值\n仅启用关联可观测性\n关联命中率\n未启用观测时无额外开销\n程序化启用后可解释调度决策\nshared-reset-not-full-workflow\nheavy-option-points-to-bench\ncolumn-or-streaming-write-positive\nwrite-attribution-output-unchanged\nnested-write-not-double-counted\nclose-save-bucket-documented"
  output-mode-api	llmanspec/specs/output-mode-api/output-mode-api.feature	"定义运行时输出语义为\"显式 sink 驱动\": 是否保留内存数据、是否写文件、以及是否同时写入(tee)都通过 sink 选择表达,而不是通过 `return_data` 等布尔参数驱动 runtime 隐式装配. 同时要求稳定的执行元数据(例如 `ExecutionResult.total_rows`)以及异常路径的 best-effort 资源清理. [scope-review-2026-07-13-c25-xlsx-ir-path-presence]"	"输出是否保留内存数据由 sink 表达\n无输出时避免构造返回列表\n允许 tee 同时写文件与显式 sink\ntotal_rows 为稳定元数据\n成功路径 sink.close 失败必须使 run 失败\n异常路径 close 不得覆盖原异常\n异常路径 best-effort 关闭 sink\n需要内存数据时显式使用内存-sink\n无文件输出且无显式-sink\nstreaming-输出同时写文件与内存-sink\ntotal-rows-不依赖是否保留内存数据\nrun-成功但-close-失败导致-run-ir-失败\nengine-run-抛异常且-close-同时失败\nsink-write-触发异常"
...
properties[96	]{entry_id	name	required	summary}:
