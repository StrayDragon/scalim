...
- **0.10.0 性能三项**（write-precompute / fusion 默认开、chunk 并行 Python opt-in）本身不是 YAML breaking。
- **c40 追加**：YAML `lookup_chunk_size` 已迁出 → `LookupChunking`；下游盘点须扫该字段并改 Python（`references/upgrades/2026-08-09-lookup-chunking-python-ssot.md`）。其余仍以「观测订阅是否关掉 fusion / 是否误开并行」为主。入口:`references/0.10-release-highlights.md`。
...
- 对每一类错误,用 upgrades 文档找到对应的迁移批次与迁移方式:
  ...
  - book `write_defaults` / 旧 Override 类型: 优先 `references/upgrades/2026-07-12-book-write-policy-python-ssot.md`；book `budget` / `BookBudgetPolicy` 删除见 `2026-07-28-remove-book-budget-policy.md`
  - Python IR `DedupBy*` / `TwoStageGroupBy*`: `references/upgrades/2026-07-28-remove-dedup-and-two-stage-derived.md`
  - 旧 `xlsx_file` / `xlsx_memory`（现已硬删 fail-fast）→ 唯一 `xlsx`(可选 `path`): 优先 `references/upgrades/2026-07-20-remove-deprecated-xlsx-file-memory-kinds.md`（历史过渡说明: `2026-07-13-unified-xlsx-book-kind.md`；IR pathful/pathless: `2026-07-13-normalize-xlsx-book-ir-path-presence.md`）
