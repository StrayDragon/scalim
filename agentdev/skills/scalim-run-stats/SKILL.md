...
- 装配 profile / 读 `nodes[]` / 落盘 sibling：读 [references/task-assemble-and-compare.md](references/task-assemble-and-compare.md)
...
2. **日常用 bench**：`ObservabilityProfile.BENCH`；debug / relation / field_compute top-N / viz_trace|full 会 warn，且观测税更高。
...
8. **对拍**：baseline vs bench 业务输出（CSV 行/内容哈希）MUST 不变；可变的是墙钟与 metrics。
