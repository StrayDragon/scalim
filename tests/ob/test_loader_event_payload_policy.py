...
def _emit_event(policy: LoaderResultPolicy, sample_size: int, result) -> object:  # type: ignore[no-untyped-def]
...
def test_loader_event_payload_full() -> None:
    result = [1, 2, 3]
    event = _emit_event(LoaderResultPolicy.FULL, 2, result)
...
def test_loader_event_payload_summary() -> None:
    result = [1, 2, 3, 4]
    event = _emit_event(LoaderResultPolicy.SUMMARY, 2, result)
...
def test_loader_event_payload_sample() -> None:
    result = [1, 2, 3]
    event = _emit_event(LoaderResultPolicy.SAMPLE, 2, result)
...
def test_loader_event_payload_none() -> None:
    result = [1]
    event = _emit_event(LoaderResultPolicy.NONE, 2, result)
...
def test_loader_event_payload_invalid_policy_raises() -> None:
    with pytest.raises(TypeError, match=r"loader_result_policy must be a LoaderResultPolicy"):
        ObserverManager(loader_result_policy="bad")  # type: ignore[arg-type]


def test_loader_event_payload_sample_mapping() -> None:
    result = {"a": 1, "b": 2}
    event = _emit_event(LoaderResultPolicy.SAMPLE, 1, result)
...
def test_loader_event_payload_sample_string() -> None:
    result = "abcdef"
    event = _emit_event(LoaderResultPolicy.SAMPLE, 3, result)
