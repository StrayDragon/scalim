...
def test_performance_observer_batch_lines_use_structured_jsonl_when_installed() -> None:
    ...
    with _installed_jsonl(buf):
        ...
        obs.on_operator_span(event_envelope(OperatorSpanEvent(operator_type="load", field_key="a", batch_num=1, duration=0.1)))
        obs.on_operator_span(event_envelope(OperatorSpanEvent(operator_type="compute", field_key=None, batch_num=1, duration=0.1)))
