...
def test_adaptive_loadref_parallelism_replays_in_plan_order_on_main_thread() -> None:
    ...
    sleep_by_name: Dict[str, float] = {"customers": 0.0, "products": 0.0}
    ...
    def _load_customers(customer_id_set=None):  # type: ignore[no-untyped-def]
        _barrier("customers")
        time.sleep(sleep_by_name["customers"])
        with completion_lock:
            completion_order.append("customers")
        _ = customer_id_set
        return {
    ...
    def _load_products(product_id_set=None):  # type: ignore[no-untyped-def]
        _barrier("products")
        time.sleep(sleep_by_name["products"])
        with completion_lock:
            completion_order.append("products")
        _ = product_id_set
        return {
    ...
    sleep_by_name[expected[0]] = 0.05
    sleep_by_name[expected[1]] = 0.01
