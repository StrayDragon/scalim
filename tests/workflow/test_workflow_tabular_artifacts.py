...
def test_workflow_resource_manager_rejects_rows_for_xlsx_file() -> None:
    with pytest.raises(ScalimWorkflowWriteError, match="only accept CSV-equivalent artifacts"):
        _ = WorkflowResourceManager._require_csv_input(InMemoryRows(header=["id"], rows=[[1]]))
