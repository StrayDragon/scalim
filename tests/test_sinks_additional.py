...
from scalim.sinks.sink_csv import CSVSink, ColumnCSVSink, BlockColumnCSVSink
...
def test_csv_sink_close_replace_exception_unlink_failure_logs_warning(tmp_path: Path, monkeypatch, caplog) -> None:
    ...
    assert any("CSVSink failed to remove temp file" in record.message for record in caplog.records)
    assert temp_path.exists()
...
def test_column_csv_sink_close_replace_exception_unlink_failure_logs_warning(tmp_path: Path, monkeypatch, caplog) -> None:
    ...
    assert any("ColumnCSVSink failed to remove temp file" in record.message for record in caplog.records)
