...
from scalim.dsl.by_yaml.config_parsing.field_index import CollectingOutputFieldErrors, OutputFieldResolver, build_source_data_key_index
from scalim.dsl.by_yaml.config_parsing.models import AliasIndex, FieldDef, RawDemand
...
def test_validator_validate_main_source_edges() -> None:
    ...
    validator._validate_main_source(
        {
            "main_source": {"source_id": "orders", "loader": "tests.conftest.mock_loader"},
            "sources": {"orders": {"loader": "tests.conftest.mock_loader", "key": "id"}},
        },
        errors,
    )
...
def test_validator_output_field_duplicate_defs() -> None:
    validator = validator_module.ConfigValidator()
    field_a = _field_def("dup", source_id="orders", field="id")
    field_b = _field_def("dup", source_id="s1", field="id2")
    defs_by_id = {"dup": [field_a, field_b]}
    errors = []
    config = {"output": {"fields": [{"field_id": "dup", "source": "orders"}, {"field_id": "dup", "source": "s1"}]}}
...
def test_validator_output_field_helpers() -> None:
    errors = []
    resolver = OutputFieldResolver({}, AliasIndex(), CollectingOutputFieldErrors(errors))
    field_def, _override, _entry_kind = resolver.resolve_entry({"field_id": "missing"}, 0)
    assert field_def is None
    assert any("Output field 'missing' not found" in msg for msg in _messages(errors))

    dup_a = _field_def("dup", source_id="a", field="id")
    dup_b = _field_def("dup", source_id="b", field="id")
    alias_index = AliasIndex()
    alias_index.add(dup_a.data, dup_a)
    alias_index.add(dup_b.data, dup_b)

    errors = []
    alias_index = AliasIndex()
    alias_index.add(dup_a.data, dup_a)
    alias_index.add(dup_b.data, dup_b)
    resolver = OutputFieldResolver({"dup": [dup_a, dup_b]}, alias_index, CollectingOutputFieldErrors(errors))
    resolver.resolve_entry({"field_id": "dup"}, 0)
    assert any("add source to explicit field_id object" in msg for msg in _messages(errors))

    errors = []
    resolver = OutputFieldResolver({"dup": [dup_a]}, alias_index, CollectingOutputFieldErrors(errors))
    resolver.resolve_entry({"field_id": "dup", "source": "missing"}, 0)
    assert any("has no match for source" in msg for msg in _messages(errors))

    errors = []
    dup_same_source_a = _field_def("dup", source_id="a", field="id")
    dup_same_source_b = _field_def("dup", source_id="a", field="id2")
    alias_index = AliasIndex()
    alias_index.add(dup_same_source_a.data, dup_same_source_a)
    alias_index.add(dup_same_source_b.data, dup_same_source_b)
    resolver = OutputFieldResolver(
        {"dup": [dup_same_source_a, dup_same_source_b]},
        alias_index,
        CollectingOutputFieldErrors(errors),
    )
    result, _override, _entry_kind = resolver.resolve_entry({"field_id": "dup", "source": "a"}, 0)
    assert result is None
    assert any("use unique field_id" in msg for msg in _messages(errors))
...
def test_validator_output_field_data_key_helpers() -> None:
    ...
    same_source_dup_a = _field_def("same_source_dup_a", source_id="a", field="same")
    same_source_dup_b = _field_def("same_source_dup_b", source_id="a", field="same")
    unique = _field_def("unique", source_id="u", field="uniq")

    alias_index = AliasIndex()
    for field_def in [dup_a, dup_b, same_source_dup_a, same_source_dup_b, unique]:
        alias_index.add(field_def.data, field_def)

    defs_by_id = {
        "dup_a": [dup_a],
        "dup_b": [dup_b],
        "same_source_dup_a": [same_source_dup_a],
        "same_source_dup_b": [same_source_dup_b],
        "unique": [unique],
    }
    defs_by_data_key_by_source = build_source_data_key_index([dup_a, dup_b, same_source_dup_a, same_source_dup_b, unique])
    ...
    field_def, _override, _entry_kind = resolver.resolve_entry({"field": None}, 0)
    assert field_def is None
    assert any("output.fields[0] missing field; use explicit field object" in msg for msg in _messages(errors))
    ...
    field_def, _override, _entry_kind = resolver.resolve_entry({"field": "missing"}, 0)
    assert field_def is None
    assert any("Output field data_key 'missing' not found" in msg for msg in _messages(errors))
    ...
    field_def, _override, _entry_kind = resolver.resolve_entry({"field": "id", "source": "missing"}, 0)
    assert field_def is None
    assert any("Output field data_key 'id' has no match for source 'missing'" in msg for msg in _messages(errors))
    ...
    field_def, _override, _entry_kind = resolver.resolve_entry({"field": "id"}, 0)
    assert field_def is None
    assert any("Output field data_key 'id' is ambiguous; add source or use field_id" in msg for msg in _messages(errors))
    ...
    field_def, _override, _entry_kind = resolver.resolve_entry({"field": "same", "source": "a"}, 0)
    assert field_def is None
    assert any("Output field data_key 'same' is ambiguous in source 'a'; use field_id" in msg for msg in _messages(errors))
    ...
    field_def, _override, _entry_kind = resolver.resolve_entry({"field": "same"}, 0)
    assert field_def is None
    assert any("Output field data_key 'same' is ambiguous in source 'a'; use field_id" in msg for msg in _messages(errors))
    ...
    field_def, override, _entry_kind = resolver.resolve_entry({"field": "id", "source": "a", "name": "ID"}, 0)
    assert field_def is dup_a
    assert override == {"name": "ID"}
    ...
    field_def, override, _entry_kind = resolver.resolve_entry({"field": "uniq"}, 0)
    assert field_def is unique
    assert override is None


def test_validator_output_field_source_ambiguity_duplicates() -> None:
    validator = validator_module.ConfigValidator()
    duplicate_fields_by_source = {}
    seen_field_values_by_source = {}
...
def test_output_field_issue_collector_tracks_paths() -> None:
    errors = []
    collector = validator_module._OutputFieldIssueCollector(errors, "output.fields")
    collector.set_index(2)
    collector.type_error("bad")
    collector.value_error("worse")
    collector.error("oops")

    assert [issue.path for issue in errors] == ["output.fields.2", "output.fields.2", "output.fields.2"]
