...
class TestDependencyGraph:
    ...
    def test_detect_cycles_skips_unknown_nodes_from_iter(self, monkeypatch) -> None:
        ...
        def _fake_set(iterable=None):  # type: ignore[no-untyped-def]
            nonlocal calls
            calls += 1
            if iterable is None:
                iterable = []
            s = _CustomSet(iterable)
            if calls == 1:
                s.force_false = True
            return s

        monkeypatch.setattr("scalim.utils.graph.set", _fake_set, raising=False)
        cycles = detect_cycles([object()], lambda _n: [])
        assert cycles == []
