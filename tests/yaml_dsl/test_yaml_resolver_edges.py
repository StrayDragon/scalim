...
@pytest.mark.parametrize(
    "allowed_fn",
    [
        "tests.fixtures.resolver_allowlist_mod:Obj.safe",
        "tests.fixtures.resolver_allowlist_mod.Obj.safe",
    ],
...
def test_resolver_class_style_allowlist_matches_full_attr_chain(allowed_fn: str) -> None:
    ...
    safe = resolver.resolve("tests.fixtures.resolver_allowlist_mod:Obj.safe")
    assert safe() == "safe"

    with pytest.raises(ScalimResolverError, match="Obj.unsafe"):
        resolver.resolve("tests.fixtures.resolver_allowlist_mod:Obj.unsafe")


def test_resolver_allowed_modules_allows_class_style_methods() -> None:
    resolver = PythonReferenceResolver(allowed_modules=frozenset(["tests.fixtures.resolver_allowlist_mod"]))

    safe = resolver.resolve("tests.fixtures.resolver_allowlist_mod:Obj.safe")
    unsafe = resolver.resolve("tests.fixtures.resolver_allowlist_mod:Obj.unsafe")
