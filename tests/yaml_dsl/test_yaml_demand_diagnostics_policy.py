...
""".lstrip()
    ...
    compilation = compile(str(yaml_path), options=DemandRunOptions(security=DemandRunSecurityOptions(allowed_modules=_ALLOWED_MODULES)))
    ...
    compilation_full = compile(
        ...
        options=DemandRunOptions(
            ...
            runtime=DemandRunRuntimeOptions(demand_diagnostics=DemandDiagnosticsPolicy(include_full_error_message=True)),
        ),
    )
...
def test_workflow_run_options_patch_demand_diagnostics_none_disables_policy() -> None:
    from scalim.dsl.yaml_dsl.workflow_entrypoints import _apply_workflow_node_patch_demand_diagnostics
    from scalim.dsl.yaml_dsl.workflow_types import WorkflowNodePatch

    base = DemandRunOptions(
        security=DemandRunSecurityOptions(allowed_modules=_ALLOWED_MODULES),
    ...
    patch = WorkflowNodePatch(demand_diagnostics=None)

    next_options = _apply_workflow_node_patch_demand_diagnostics(base, patch)
    assert next_options.runtime.demand_diagnostics is None
...
def test_workflow_run_options_patch_demand_diagnostics_invalid_type_rejected() -> None:
    ...
    base = DemandRunOptions(security=DemandRunSecurityOptions(allowed_modules=_ALLOWED_MODULES))
    patch = WorkflowNodePatch(demand_diagnostics="bad")  # type: ignore[arg-type]

    with pytest.raises(TypeError, match=r"WorkflowNodePatch\.demand_diagnostics must be a DemandDiagnosticsOverride or None"):
        _ = _apply_workflow_node_patch_demand_diagnostics(base, patch)
...
def test_workflow_run_options_patch_demand_diagnostics_all_unset_is_noop() -> None:
    ...
    patch = WorkflowNodePatch(demand_diagnostics=DemandDiagnosticsOverride())
...
def test_workflow_run_options_patch_demand_diagnostics_merges_fields_with_unset_inheritance() -> None:
    ...
    patch = WorkflowNodePatch(demand_diagnostics=DemandDiagnosticsOverride(validate_unique_field_names=False))

    next_options = _apply_workflow_node_patch_demand_diagnostics(base, patch)
    assert next_options.runtime.demand_diagnostics is not None
    assert next_options.runtime.demand_diagnostics.include_full_error_message is True
    assert next_options.runtime.demand_diagnostics.validate_unique_field_names is False
