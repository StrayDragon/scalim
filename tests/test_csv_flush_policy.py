...
def test_csv_sink_flush_policy_every_n_rows(tmp_path, monkeypatch) -> None:
    state = {"count": 0}
    ...
    monkeypatch.setattr("scalim.sinks.sink_csv.io.open", _open)
...
def test_csv_sink_flush_policy_every_n_rows_batch(tmp_path, monkeypatch) -> None:
    state = {"count": 0}
    original_open = io.open
...
def test_csv_sink_flush_policy_always_flushes_on_write_row(tmp_path, monkeypatch) -> None:
    state = {"count": 0}
    original_open = io.open
