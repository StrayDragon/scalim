...
@pytest.mark.parametrize(
    "spec_kwargs, match",
    [
        ({"enabled": 1}, r"enabled must be a bool"),
        ({"max_attempts": True}, r"max_attempts must be an int"),
        ({"max_elapsed_seconds": True}, r"max_elapsed_seconds must be a number"),
        ({"base_delay_seconds": True}, r"base_delay_seconds must be a number"),
        ({"max_delay_seconds": True}, r"max_delay_seconds must be a number"),
        ({"jitter": 1}, r"jitter must be a bool"),
    ],
)
def test_merge_loader_retry_policy_does_not_silently_coerce_types(spec_kwargs, match) -> None:  # type: ignore[no-untyped-def]
    base = LoaderRetryPolicy.disabled()
    spec = LoaderRetryPolicySpec(**spec_kwargs)

    with pytest.raises(TypeError, match=match):
        _ = merge_loader_retry_policy(base, spec)
