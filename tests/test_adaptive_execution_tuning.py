...
def test_adaptive_scheduler_decision_events_are_wants_gated(monkeypatch) -> None:
    source_a = _make_source("s1")
    source_b = _make_source("s2")
    op_a = _make_loadref_op(field_key="a", to_source=source_a, lookup_steps=(LookupStepIr(from_field="id", to_source=source_a),))
    op_b = _make_loadref_op(field_key="b", to_source=source_b, lookup_steps=(LookupStepIr(from_field="id", to_source=source_b),))
    ...
    monkeypatch.setattr(
        ...
        "emit_lazy",
        lambda event_type, payload_factory, meta=None: (_ for _ in ()).throw(AssertionError("emit_lazy called")),  # type: ignore[no-untyped-def]
        raising=True,
    )
...
def test_adaptive_scheduler_emits_pool_wait_stats_when_subscribed() -> None:
    source_a = _make_source("s1")
    source_b = _make_source("s2")
    source_c = _make_source("s3")
...
def test_adaptive_scheduler_process_backend_incompatible_hooks_emits_serial_decision_when_subscribed(monkeypatch) -> None:
    class _ProcessPolicy(AdaptivePolicy):
        def choose_backend(self, *, plan, runtime, tuning):  # type: ignore[override]
            _ = plan
            _ = runtime
