...
def test_adaptive_scheduler_process_backend_invalid_failure_mode_raises() -> None:
    ...
    op_b = _make_loadref_op(field_key="b", to_source=source_b, lookup_steps=(LookupStepIr(from_field="id", to_source=source_b),))
    plan = ExecutionPlan(operators=(op_a, op_b))
    scheduler = AdaptiveLoadRefScheduler(plan, overrides=PipelineOverrides(adaptive_policy=_ProcessPolicy()))
    runtime = ExecutionRuntime(plan, HookManager(), ObserverManager(), main_source=None)

    with pytest.raises(ValueError, match="Invalid process failure mode"):
        scheduler.execute_segment(
            [op_a, op_b],
            context=BatchContext(),
            batch_row_nth=[0],
...
def test_adaptive_scheduler_process_backend_incompatible_hooks_fail_fast_raises() -> None:
    class _ProcessPolicy(AdaptivePolicy):
        def choose_backend(self, *, plan, runtime, tuning):  # type: ignore[override]
            _ = plan
            _ = runtime
    ...
    hooks = HookManager()
    hooks.register(_NoopHook())
    runtime = ExecutionRuntime(plan, hooks, ObserverManager(), main_source=None)
...
def test_adaptive_scheduler_process_backend_pickle_fail_fast_raises_typeerror() -> None:
    class _ProcessPolicy(AdaptivePolicy):
        def choose_backend(self, *, plan, runtime, tuning):  # type: ignore[override]
            _ = plan
            _ = runtime
...
def test_adaptive_scheduler_process_backend_pickle_fallback_serial(monkeypatch) -> None:
    class _ProcessPolicy(AdaptivePolicy):
        def choose_backend(self, *, plan, runtime, tuning):  # type: ignore[override]
            _ = plan
            _ = runtime
        ...
        def choose_process_failure_mode(self, *, plan, runtime, tuning):  # type: ignore[override]
            ...
            _ = runtime
            _ = tuning
            return PROCESS_FAILURE_FALLBACK_SERIAL
    ...
    scheduler = AdaptiveLoadRefScheduler(
        plan,
        overrides=PipelineOverrides(
            adaptive_policy=_ProcessPolicy(), adaptive_loadref_executor_factory=lambda: RecordingLoadRefExecutor(calls)
        ),
    )
    runtime = ExecutionRuntime(plan, HookManager(), ObserverManager(), main_source=None)
    # Keep this mock: process backend forbids real hooks/observers, so wants=True cannot be reached naturally.
    monkeypatch.setattr(runtime.instrumentation, "wants", lambda _event_type: True, raising=True)  # type: ignore[arg-type]
...
def test_adaptive_scheduler_process_backend_incompatible_hooks_emits_serial_decision_when_subscribed() -> None:
    class _ProcessPolicy(AdaptivePolicy):
        def choose_backend(self, *, plan, runtime, tuning):  # type: ignore[override]
            _ = plan
            _ = runtime
    ...
    scheduler = AdaptiveLoadRefScheduler(
        plan,
        overrides=PipelineOverrides(adaptive_policy=_ProcessPolicy(), adaptive_loadref_executor_factory=NoOpLoadRefExecutor),
    )
    ...
    runtime = ExecutionRuntime(plan, hooks, ObserverManager(observers=[perf]), main_source=None)
    ...
    assert perf.metrics.adaptive_scheduler is not None
    assert perf.metrics.adaptive_scheduler.serial_reasons.get("process_backend_incompatible_hooks") == 1


def test_adaptive_scheduler_process_backend_unpicklable_task_falls_back_to_serial() -> None:
    class _ProcessPolicy(AdaptivePolicy):
        def choose_backend(self, *, plan, runtime, tuning):  # type: ignore[override]
            _ = plan
            _ = runtime
    ...
    source_a = SourceIr(source_id="s1", key=KeyIr(key="id"), loader_spec=LoaderIr(callable=_loader_a))
    source_b = SourceIr(source_id="s2", key=KeyIr(key="id"), loader_spec=LoaderIr(callable=_loader_b))
    op_a = _make_loadref_op(field_key="a", to_source=source_a, lookup_steps=(LookupStepIr(from_field="id", to_source=source_a),))
    op_b = _make_loadref_op(field_key="b", to_source=source_b, lookup_steps=(LookupStepIr(from_field="id", to_source=source_b),))
    plan = ExecutionPlan(operators=(op_a, op_b))
    scheduler = AdaptiveLoadRefScheduler(plan, overrides=PipelineOverrides(adaptive_policy=_ProcessPolicy(), adaptive_min_parallel_tasks=1))
    runtime = ExecutionRuntime(plan, HookManager(), ObserverManager(), main_source=MainSourceIr(source_id="main", loader=_main_loader))

    context = BatchContext(on_field_set=lambda *_args, **_kwargs: None)
...
def test_adaptive_scheduler_process_backend_executes_inline_when_pickle_check_passes() -> None:
    class _ProcessPolicy(AdaptivePolicy):
        def choose_backend(self, *, plan, runtime, tuning):  # type: ignore[override]
            _ = plan
            _ = runtime
    ...
    assert ctx.get_field_value("a", 0) == "a"
    assert ctx.get_field_value("b", 0) == "b"


def test_adaptive_scheduler_process_backend_unpicklable_task_fail_fast_raises(monkeypatch) -> None:
    class _ProcessPolicy(AdaptivePolicy):
        def choose_backend(self, *, plan, runtime, tuning):  # type: ignore[override]
            _ = plan
            _ = runtime
    ...
    import scalim.execution.adaptive.loadref_scheduler as loadref_scheduler

    def _fake_dumps(obj: object) -> bytes:
        if isinstance(obj, tuple) and len(obj) == 3:
            raise RuntimeError("boom")
        return b"ok"

    monkeypatch.setattr(loadref_scheduler.pickle, "dumps", _fake_dumps, raising=True)
    ...
    with pytest.raises(TypeError, match="cannot pickle task"):
        scheduler.execute_segment(
            [op_a, op_b],
            context=ctx,
            batch_row_nth=[0],
...
def test_adaptive_scheduler_process_backend_unpicklable_task_falls_back_to_serial_on_task_pickle_failure(monkeypatch) -> None:
    class _ProcessPolicy(AdaptivePolicy):
        def choose_backend(self, *, plan, runtime, tuning):  # type: ignore[override]
            _ = plan
            _ = runtime
...
def test_adaptive_scheduler_process_backend_pickle_check_does_not_repeat_shared_objects(monkeypatch) -> None:
    class _ProcessPolicy(AdaptivePolicy):
        def choose_backend(self, *, plan, runtime, tuning):  # type: ignore[override]
            _ = plan
            _ = runtime
    ...
    def _recording_dumps(obj: object) -> bytes:
        dumps_calls.append(obj)
        return b"ok"

    monkeypatch.setattr(loadref_scheduler.pickle, "dumps", _recording_dumps, raising=True)
    ...
    assert dumps_calls
    shared_check = dumps_calls[0]
    assert isinstance(shared_check, tuple)
    assert any(item is plan for item in shared_check)
    assert any(item is ctx for item in shared_check)

    for item in dumps_calls[1:]:
        assert isinstance(item, tuple)
        assert not any(elem is plan for elem in item)
        assert not any(elem is ctx for elem in item)
